...
async fn execute_tunnel_command(agent: &SharedAgent, cmd: TunnelControlCommand) -> String {
    ...
    let client = match crate::core::runtime::net::build_internal_control_client(10) {
        Ok(c) => c,
...
fn parse_set_secret(text: &str) -> Option<(String, String)> {
    crate::core::runtime::secrets::parse_set_secret_command(text)
}
...
fn parse_use_current_llm_key(text: &str) -> Option<String> {
    ...
    crate::core::runtime::secrets::parse_use_current_llm_key_command(trimmed)
}
...
async fn store_secret_for_chat(agent: &SharedAgent, key: &str, value: &str) -> Result<(), String> {
    ...
    tokio::task::spawn_blocking(move || -> Result<(), String> {
        crate::core::runtime::secrets::store_user_secret(&config_dir, Some(&data_dir), &k, &v)
...
fn http_client() -> reqwest::Client {
    crate::core::runtime::net::default_outgoing_http_client()
}
    ...
    if let Some((key, value)) = parse_set_secret(&text) {
        if !crate::core::runtime::secrets::setsecret_command_escape_hatch_enabled() {
            send_reply(
                &config,
                from,
                crate::core::runtime::secrets::setsecret_command_disabled_response(),
            )
            .await?;
            return Ok("ok".to_string());
        }
...
async fn handle_command(text: &str, agent: &SharedAgent, from: &str) -> String {
    ...
    match command {
        ...
        "/setsecret" => {
            if !crate::core::runtime::secrets::setsecret_command_escape_hatch_enabled() {
                return crate::core::runtime::secrets::setsecret_command_disabled_response()
                    .to_string();
            }
