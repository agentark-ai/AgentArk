...
pub(super) async fn control_plane_tunnel_is_active(state: &AppState) -> bool {
    ...
    tunnel.active
        && tunnel.selected_app_id.is_none()
        && tunnel.exposed_app_ids.is_empty()
...
async fn request_matches_active_control_plane_tunnel(
    ...
    let (active, tunnel_url, selected_app_id, exposed_app_ids_empty, control_plane_enabled) = {
        let tunnel = state.tunnel.read().await;
        (
            tunnel.active,
            tunnel.url.clone(),
