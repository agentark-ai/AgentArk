...
pub(super) async fn list_available_channels(State(state): State<AppState>) -> Response {
    ...
    let config_manager = match crate::core::runtime::config::SecureConfigManager::new_with_data_dir(
        &agent.config_dir,
        Some(&agent.data_dir),
    ) {
...
pub(super) async fn memory_stats(
    ...
    let assistant_preference_count = agent
        ...
        .count_facts_by_category(
            project_id,
            crate::core::knowledge::memory_schema::MEMORY_CATEGORY_ASSISTANT_PREFERENCE,
        )
        .await
        .unwrap_or(0);
    ...
    let other_memory_count = agent
        ...
        .count_facts_by_category(
            project_id,
            crate::core::knowledge::memory_schema::MEMORY_CATEGORY_OTHER,
        )
        .await
        .unwrap_or(0);
...
fn memory_fact_response_payload(
    ...
    serde_json::json!({
        ...
        "memory_category": crate::core::knowledge::memory_schema::memory_category_from_metadata(
            &item.metadata,
            memory_kind,
        ),
        "topics": crate::core::knowledge::memory_schema::normalize_memory_topics(
        ...
        .await
    {
        Ok(item) => {
            if item.project_id.is_none() {
                ...
                if let Err(error) =
                    crate::core::knowledge::learning::sync_user_preference_to_experience_item(
                        &agent.storage,
                        &item.key,
                        &item.value,
        ...
        .await
        {
            Ok(Ok(embeddings)) if embeddings.len() == texts.len() => {
                ...
                let mut scored = examples
                    ...
                    .enumerate()
                    .map(|(index, example)| {
                        let score = crate::core::knowledge::document_search::normalized_embedding_similarity(
...
fn arkmemory_expired_memory_cleanup_item(
    ...
    Some(serde_json::json!({
        ...
        "title": format!(
            "Expired {}",
            crate::core::knowledge::memory_schema::memory_category_label(memory_category)
                .to_ascii_lowercase()
        ),
...
async fn arkmemory_expire_stale_memory_items(
    ...
    for item in items {
        ...
        let category = candidate
            .get("memory_category")
            .and_then(|value| value.as_str())
            .unwrap_or(crate::core::knowledge::memory_schema::MEMORY_CATEGORY_OTHER);
        arkmemory_record_event_once(
            storage,
