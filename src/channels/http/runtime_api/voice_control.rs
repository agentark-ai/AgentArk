...
pub(super) async fn create_voice_session(
    ...
    let transport = request
        .transport
        .as_deref()
        .unwrap_or(crate::core::runtime::voice::VOICE_TRANSPORT_BROWSER)
        .trim();
    if transport != crate::core::runtime::voice::VOICE_TRANSPORT_BROWSER
        && transport != crate::core::runtime::voice::VOICE_TRANSPORT_BROWSER_WEBSOCKET
    {
    ...
    let (session, stream_token) = {
        ...
        let session = if transport == crate::core::runtime::voice::VOICE_TRANSPORT_BROWSER_WEBSOCKET
        {
            conversation_id
                .as_deref()
...
pub(super) async fn stream_voice_session(
    ...
    let stream_url = match crate::core::runtime::voice::voice_bridge_stream_url(bridge_url, &id) {
        Ok(url) => url,
