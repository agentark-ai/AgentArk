...
pub(super) fn settings_configured_email_backends(
    ...
    let has_legacy_gmail = gmail_enabled
        && crate::core::runtime::config::SecureConfigManager::new(config_dir)
            .ok()
            .and_then(|manager| manager.get_custom_secret("gmail_tokens").ok().flatten())
            .is_some_and(|value| !value.trim().is_empty());
    if has_legacy_gmail {
        backends.push(crate::core::connectivity::email_delivery::EMAIL_PROVIDER_GMAIL.to_string());
    }
    ...
    if has_workspace_gmail {
        backends.push(
            crate::core::connectivity::email_delivery::EMAIL_PROVIDER_GOOGLE_WORKSPACE.to_string(),
        );
    }
    if crate::core::connectivity::email_delivery::external_email_delivery_is_ready(&config.email) {
...
pub(super) fn build_email_settings_response(
    ...
    let provider =
        crate::core::connectivity::email_delivery::normalize_email_provider(&config.email.provider);
    let auto_resolves_to =
        if provider == crate::core::connectivity::email_delivery::EMAIL_PROVIDER_AUTO {
            crate::core::connectivity::email_delivery::normalize_email_backend_selection(
                &provider,
                &available_backends,
            )
            .ok()
    ...
    EmailSettingsResponse {
        ...
        auto_resolves_to,
        delivery_ready: crate::core::connectivity::email_delivery::email_channel_is_ready(
            &config.email.provider,
            &available_backends,
        ),
        transport: EmailTransportSettingsResponse {
            kind: crate::core::connectivity::email_delivery::normalize_transport_kind(
        ...
        auth: EmailAuthSettingsResponse {
            kind: crate::core::connectivity::email_delivery::normalize_auth_kind(
                &config.email.auth.kind,
            ),
...
fn api_key_metadata_payload(
    info: Option<&crate::core::runtime::config::HttpApiKeyInfo>,
    rotated: bool,
    include_key: bool,
) -> serde_json::Value {
    let Some(info) = info else {
        let mut payload = serde_json::json!({
            ...
            "ttl_seconds": crate::core::runtime::config::HTTP_API_KEY_TTL_SECS,
...
pub(super) async fn regenerate_api_key_endpoint(
    ...
    let secure_config = crate::core::runtime::config::SecureConfigManager::new_with_data_dir(
        &config_dir,
        Some(&data_dir),
    );
...
mod tests {
    ...
    fn api_key_metadata_payload_excludes_full_key_unless_requested() {
        let info = crate::core::runtime::config::HttpApiKeyInfo {
