...
pub(super) fn automation_watcher_status_label(
    ...
    match status {
        ...
        crate::core::automation::watcher::WatcherStatus::TimedOut => "timed_out".to_string(),
...
pub(super) fn automation_watcher_next_run_at(
    ...
    if !matches!(
        ...
        crate::core::automation::watcher::WatcherStatus::Active
            | crate::core::automation::watcher::WatcherStatus::Paused
    ) {
        return None;
    }
...
pub(super) fn collect_background_session_counts(
    ...
    session: &crate::core::BackgroundSession,
    tasks: &[Task],
    watchers: &[crate::core::automation::watcher::Watcher],
    ...
    for watcher in watchers {
        ...
        match &watcher.status {
            crate::core::automation::watcher::WatcherStatus::Active => counts.watchers_active += 1,
            crate::core::automation::watcher::WatcherStatus::Paused => counts.watchers_paused += 1,
            crate::core::automation::watcher::WatcherStatus::Triggered => {
                counts.watchers_triggered += 1
            }
...
pub(super) async fn detach_background_session_work(
    ...
    (
        ...
        Json(serde_json::json!({
            ...
            "deleted_watcher_ids": Vec::<String>::new(),
        })),
    )
        .into_response()
}
...
pub(super) async fn delete_background_session(
    ...
    for watcher_id in &session.linked_watcher_ids {
        match rebind_watcher_background_session(&state, watcher_id, None).await {
            Ok(false) => {
                tracing::debug!(
                    "Deleted background session {} referenced missing watcher {} while unlinking",
                    id,
                    watcher_id
                );
            }
...
pub(super) async fn plan_task(
    ...
    let (selector_prompt, selector_message) =
        crate::core::orchestration::planner::build_action_selector_prompt(
            &request.description,
    ...
    let needed_action_names = crate::core::orchestration::planner::parse_action_selection(
        &selector_response.content,
        &actions,
        crate::core::orchestration::planner::DEFAULT_MAX_ACTIONS_FOR_PLAN,
    );
    let needed_actions = crate::core::orchestration::planner::shortlist_actions(
        &actions,
        &needed_action_names,
        crate::core::orchestration::planner::DEFAULT_MAX_ACTIONS_FOR_PLAN,
    );

    let (plan_prompt, plan_message) = crate::core::orchestration::planner::build_plan_prompt(
        &request.description,
        request.prompt.as_deref(),
        &needed_actions,
    ...
    match crate::core::orchestration::planner::parse_plan_from_llm_content(
...
pub(super) fn recommendation(
    ...
    let readiness = crate::core::runtime::readiness::evaluate_recommended_action_readiness(
        &trust,
        readiness_policy,
    );
    RecommendedAction {
...
fn watch_payload_from_chat_suggestion(
    ...
    let semantic_description = if focus.is_empty() || focus == description {
        description.to_string()
    } else {
        format!("{}: {}", description, focus)
    };
    ...
    serde_json::json!({
        ...
        "condition": {
            "description": "Trigger when the observed target contains a meaningful new or actionable update for the accepted monitoring goal.",
        ...
        "conversation_id": suggestion.conversation_id,
        "source": {
            "kind": "chat_automation_suggestion",
            "focus": focus,
...
pub(super) async fn execute_accepted_watcher_suggestion(
    ...
    let payload = watch_payload_from_chat_suggestion(suggestion, &focus);
    ...
    if !has_watcher_id {
        let message = result
            ...
            .map(str::trim)
            .filter(|value| !value.is_empty())
            .unwrap_or("Watcher creation did not return a durable watcher id.");
...
pub(super) async fn run_recommended_action(
    ...
    let readiness = action.readiness.clone().unwrap_or_else(|| {
        crate::core::runtime::readiness::evaluate_recommended_action_readiness(
            &trust,
            &readiness_policy,
        )
    });
...
mod background_session_visibility_tests {
    ...
    fn test_chat_suggestion(kind: &str) -> ChatAutomationSuggestion {
        ChatAutomationSuggestion {
            id: "suggestion-1".to_string(),
            status: "open".to_string(),
            kind: kind.to_string(),
            title: "Gmail inbox watcher".to_string(),
            detail: "Monitor latest emails and flag important new messages.".to_string(),
            rationale: "The conversation contains a concrete durable monitoring intent."
                .to_string(),
            confidence: 0.91,
            created_at: "2026-06-06T00:00:00Z".to_string(),
            updated_at: "2026-06-06T00:00:00Z".to_string(),
            conversation_id: "conversation-1".to_string(),
            conversation_title: "Inbox monitoring".to_string(),
            conversation_channel: "web".to_string(),
            source_message_id: "message-1".to_string(),
            source_snippet: "check my latest emails".to_string(),
            fingerprint: "fingerprint-1".to_string(),
            goal_title: "Gmail inbox watcher".to_string(),
            goal_detail: Some(
                "Periodically check Gmail for recent messages and alert on important unread emails."
                    .to_string(),
            ),
    ...
    #[test]
    fn watcher_payload_from_chat_suggestion_uses_semantic_watch_contract() {
        let suggestion = test_chat_suggestion("watcher");
        let focus = chat_suggestion_execution_focus(&suggestion);
        let payload = watch_payload_from_chat_suggestion(&suggestion, &focus);

        assert_eq!(
            payload.get("poll_action").and_then(|value| value.as_str()),
            None
        );
        assert_eq!(
            payload["condition"]["evaluation_mode"].as_str(),
            Some("change")
        );
        assert_eq!(payload["until_stopped"].as_bool(), Some(true));
        assert_eq!(payload["repeat_on_match"].as_bool(), Some(true));
        assert_eq!(
            payload["_chat_suggestion"]["suggestion_id"].as_str(),
            Some("suggestion-1")
        );
        assert!(payload["description"]
            .as_str()
            .unwrap_or_default()
            .contains("Gmail inbox watcher"));
    }
