...
pub(super) async fn load_learning_enabled(storage: &crate::storage::Storage) -> bool {
    storage
        .get(crate::core::knowledge::learning::LEARNING_ENABLED_KEY)
        .await
        .ok()
        .flatten()
        .and_then(|raw| String::from_utf8(raw).ok())
...
pub(super) async fn load_learning_model_slot(storage: &crate::storage::Storage) -> Option<String> {
    storage
        .get(crate::core::knowledge::learning::LEARNING_MODEL_SLOT_KEY)
        .await
        .ok()
        .flatten()
        .and_then(|raw| String::from_utf8(raw).ok())
...
pub(super) async fn load_learning_queue_cap(storage: &crate::storage::Storage) -> u64 {
    storage
        .get(crate::core::knowledge::learning::LEARNING_QUEUE_CAP_KEY)
        .await
        .ok()
        .flatten()
        .and_then(|raw| String::from_utf8(raw).ok())
...
struct PromptOptimizationHoldoutFootprintKey {
    outcome: String,
    section_chars: usize,
    final_prompt_chars: usize,
}
...
fn prompt_optimization_holdout_footprint_key(
    case: &PromptOptimizationHoldoutCase,
) -> PromptOptimizationHoldoutFootprintKey {
    PromptOptimizationHoldoutFootprintKey {
        outcome: case.outcome.clone(),
        section_chars: case.section_chars,
        final_prompt_chars: case.final_prompt_chars,
    }
}

fn prompt_optimization_representative_holdout_cases(
    candidates: Vec<PromptOptimizationHoldoutCandidate>,
    limit: usize,
) -> Vec<PromptOptimizationHoldoutCase> {
    let mut key_to_index = BTreeMap::<PromptOptimizationHoldoutFootprintKey, usize>::new();
    let mut representatives = Vec::<PromptOptimizationHoldoutCandidate>::new();

    for mut candidate in candidates {
        candidate.case.matching_samples = candidate.case.matching_samples.max(1);
        let key = prompt_optimization_holdout_footprint_key(&candidate.case);
        if let Some(index) = key_to_index.get(&key).copied() {
            let representative = &mut representatives[index];
            representative.case.matching_samples = representative
                .case
                .matching_samples
                .saturating_add(candidate.case.matching_samples.max(1));
        } else {
            key_to_index.insert(key, representatives.len());
...
pub(super) fn aggregate_prompt_optimization_opportunity_profiles(
    ...
    let mut profiles = buckets
        ...
        .filter_map(|mut bucket| {
            ...
            let holdout_cases = prompt_optimization_representative_holdout_cases(
                bucket.holdout_candidates,
                PROMPT_OPTIMIZATION_HOLDOUT_LIMIT,
            );
...
fn prompt_optimization_terminal_queue_status(
    queue_status: &str,
...
pub(super) fn prompt_optimization_terminal_lifecycle_is_recorded(
    lifecycle: &PromptOptimizationLifecycleState,
    terminal_status: &str,
    job_status: &str,
    job_id: Option<&str>,
) -> bool {
    let lifecycle_status = lifecycle.status.trim();
    if lifecycle_status != terminal_status {
        return false;
    }
    if lifecycle.job_status.as_deref() != Some(job_status) {
        return false;
    }
    match job_id {
        Some(job_id) => lifecycle.terminal_baseline_job_id.as_deref() == Some(job_id),
        None => {
            lifecycle.terminal_baseline_job_id.as_deref() == Some("__terminal_without_job_id__")
        }
    }
}

async fn reconcile_prompt_optimization_terminal_lifecycles(
    storage: &crate::storage::Storage,
    review_state: &mut PromptOptimizationReviewState,
    summary: &PromptTelemetrySummary,
    gepa_queue: &serde_json::Value,
) -> Result<()> {
    let mut changed = false;
    for (proposal_id, entry) in review_state.iter_mut() {
        if entry.status.trim() != "approved" {
            continue;
        }
        let Some((queue_status, item)) = gepa_queue_item_for_proposal(gepa_queue, proposal_id)
        else {
            continue;
        };
        let Some((terminal_status, reason, job_status)) =
            prompt_optimization_terminal_queue_status(queue_status, item)
        else {
            continue;
        };
        let job = item.get("job").unwrap_or(item);
        let job_id = job
            ...
            .map(str::trim)
            .filter(|value| !value.is_empty());
        if prompt_optimization_terminal_lifecycle_is_recorded(
            &entry.lifecycle,
            &terminal_status,
            &job_status,
            job_id,
        ...
        entry.lifecycle.status = terminal_status;
        entry.lifecycle.reason = reason;
        entry.lifecycle.job_status = Some(job_status);
        entry.lifecycle.queued_job_id = job_id
            .map(str::to_string)
            .or_else(|| entry.lifecycle.queued_job_id.clone());
        entry.lifecycle.terminal_baseline_job_id = Some(
            job_id
                .map(str::to_string)
                .unwrap_or_else(|| "__terminal_without_job_id__".to_string()),
        );
        entry.lifecycle.queued_at = job
            .get("created_at")
            .and_then(|value| value.as_str())
            .map(str::to_string)
            .or_else(|| entry.lifecycle.queued_at.clone());
        entry.lifecycle.sample_baseline = summary.sample_count;
        entry.lifecycle.sample_count = 0;
        entry.lifecycle.rollback_available = false;
        changed = true;
    }
    if changed {
        storage
            .set(
                PROMPT_OPTIMIZATION_REVIEW_STATE_KEY,
                &serde_json::to_vec(review_state)?,
            )
            .await?;
    }
    Ok(())
...
pub(super) fn prompt_optimization_next_sample_baseline(
    stored_sample_baseline: usize,
    lifecycle_status: &str,
    current_sample_count: usize,
    queued_new_work: bool,
) -> usize {
    if !queued_new_work {
        return stored_sample_baseline;
    }
    if stored_sample_baseline == 0 {
        return current_sample_count;
    }
    match lifecycle_status.trim() {
        "candidate_rejected" | "blocked" | "test_stopped" | "rolled_back" => current_sample_count,
        _ => stored_sample_baseline,
    }
}

pub(super) fn prompt_optimization_terminal_retry_sample_gap(
    lifecycle: &PromptOptimizationLifecycleState,
    current_sample_count: usize,
) -> Option<(usize, usize)> {
    let status = lifecycle.status.trim();
    ...
    let required =
        prompt_optimization_lifecycle_required_samples_from_target(lifecycle.required_samples, 0);
    let fresh_samples = current_sample_count.saturating_sub(lifecycle.sample_baseline);
    (fresh_samples < required).then_some((fresh_samples, required))
}
    ...
    match surface.trim().to_ascii_lowercase().as_str() {
        ...
        "prompt_fragment" | "prompt-fragment" | "prompt-fragments" | "prompt_fragment_bundle" => {
            Ok(PromptRuntimeSurface {
                ...
                profile_key: crate::core::model::prompt_fragments::PROMPT_FRAGMENT_BUNDLE_PROFILE_KEY,
                canary_profile_key:
...
pub(super) async fn build_evolution_settings_response(
    ...
    let prompt_fragment_canary = if let Some(state) = prompt_fragment_canary_state.as_ref() {
        ...
        EvolutionCanarySummary {
            enabled: false,
            rollout_percent: 0,
            baseline_version: crate::core::model::prompt_fragments::compose_prompt_fragment_version(
    ...
    let prompt_fragment_rollback_available = storage
        .get(crate::core::model::prompt_fragments::PROMPT_FRAGMENT_BUNDLE_BASELINE_SNAPSHOT_KEY)
        .await
        .ok()
        .flatten()
        .is_some();
...
pub(super) async fn build_evolution_dev_response(
    ...
    for candidate in &learning_candidate_rows {
        let readiness = crate::core::runtime::readiness::evaluate_learning_candidate_readiness(
            candidate,
            learning_candidate_replay_gates.get(&candidate.id),
            &readiness_policy,
        );
        if let Err(error) = crate::core::runtime::readiness::record_readiness_evaluation(
            storage,
            "learning_candidate",
            &candidate.id,
            &readiness,
    ...
    for pattern in &learning_pattern_rows {
        let readiness = crate::core::runtime::readiness::evaluate_procedural_pattern_readiness(
            pattern,
            &readiness_policy,
        );
        if let Err(error) = crate::core::runtime::readiness::record_readiness_evaluation(
    ...
    let mut prompt_optimization_review_state = load_prompt_optimization_review_state(storage).await;
    if let Err(error) = reconcile_prompt_optimization_terminal_lifecycles(
        storage,
        &mut prompt_optimization_review_state,
    ...
    .await
    {
        tracing::warn!(
            error = %error,
            "Failed to reconcile prompt optimization terminal lifecycle state"
        );
    }
...
pub(super) async fn update_evolution_settings(
    ...
    if let Some(enabled) = request.self_evolve_enabled.or(request.learning_enabled) {
        ...
        if let Err(e) = store_bool_setting(
            &storage,
            crate::core::knowledge::learning::LEARNING_ENABLED_KEY,
            enabled,
        )
        .await
        {
    ...
    if let Some(slot) = request.learning_model_slot.as_deref() {
        if let Err(e) = storage
            .set(
                crate::core::knowledge::learning::LEARNING_MODEL_SLOT_KEY,
                slot.trim().as_bytes(),
            )
            .await
        {
    ...
    if let Some(cap) = request.learning_queue_cap {
        ...
        if let Err(e) = storage
            .set(
                crate::core::knowledge::learning::LEARNING_QUEUE_CAP_KEY,
                cap_value.as_bytes(),
            )
            .await
        {
    ...
    if let Some(policy) = request.readiness_policy.as_ref() {
        if let Err(e) =
            crate::core::runtime::readiness::save_readiness_policy(&storage, policy).await
        {
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(ErrorResponse {
...
pub(super) fn summarize_autonomy_action_result(
    ...
    match kind.as_str() {
        ...
        "watch" => {
            ...
            } else if !watcher_ids.is_empty() {
                format!("Created {} watcher(s).", watcher_ids.len())
            } else {
                let message = result
                    .get("message")
                    .and_then(|value| value.as_str())
                    .map(str::trim)
                    .filter(|value| !value.is_empty())
                    .unwrap_or("no durable watcher id was returned");
                format!("Watcher was not created: {}", message)
            }
        }
        "activate_mode" => {
    ...
    let message = match action.as_str() {
        ...
        "approve_learning_candidate" => {
            ...
            let readiness_policy =
                crate::core::runtime::readiness::load_readiness_policy(&storage).await;
            let readiness = crate::core::runtime::readiness::evaluate_learning_candidate_readiness(
                &candidate,
                Some(&replay_gate),
                &readiness_policy,
        ...
        "approve_prompt_optimization_proposal" => {
            ...
            let mut prompt_optimization_review_state =
                load_prompt_optimization_review_state(&storage).await;
            let terminal_summary = PromptTelemetrySummary {
                sample_count: sample_baseline,
                ...
                })
            {
                return Json(serde_json::json!({
                    "status": "waiting_for_more_samples",
                    "message": format!(
                        "This optimizer already finished without promotion. Collect {} more fresh prompt telemetry sample{} before retrying this background test.",
                        required_samples.saturating_sub(fresh_samples),
                        if required_samples.saturating_sub(fresh_samples) == 1 { "" } else { "s" }
                    ),
                ...
                if let Some(blocker) = classify_gepa_auto_readiness_blocker(&readiness) {
                        ...
                        .await
                    {
                        Ok(path) => {
                            ...
                            queued_new_prompt_optimization = true;
                            message.push_str(
            ...
            if let Err(error) = update_prompt_optimization_lifecycle(
                ...
                lifecycle_reason,
                |lifecycle| {
                    let previous_lifecycle_status = lifecycle.status.clone();
                    ...
                    lifecycle.sample_baseline = prompt_optimization_next_sample_baseline(
                        lifecycle.sample_baseline,
                        previous_lifecycle_status.as_str(),
                        sample_baseline,
                        queued_new_prompt_optimization,
                    );
