...
pub(crate) async fn load_background_learning_feed(
    ...
    let learning_enabled = crate::core::knowledge::learning::load_learning_enabled(storage).await;
...
fn should_send_standalone_sentinel_daily_review_notice(
    _settings: &AutonomySettings,
    _open_proposals: usize,
) -> bool {
    // Sentinel activity belongs in Mission Control and the daily digest, not as a standalone push.
    false
}
...
fn build_integration_prompt(
    item: &crate::core::connectivity::integration_sync::IntegrationSyncFeedItem,
) -> String {
    format!(
...
async fn build_candidates(
    storage: &crate::storage::Storage,
    integration_ctx: &crate::core::connectivity::integration_sync::IntegrationSyncContext,
    settings: &AutonomySettings,
) -> SentinelCandidateBatch {
    let now = now_rfc3339();
    let readiness_policy = crate::core::runtime::readiness::load_readiness_policy(storage).await;
    ...
    if settings.sentinel.watch_connected_services {
        let statuses =
            crate::core::connectivity::integration_sync::list_statuses(integration_ctx).await;
        batch.connected_services = statuses
            .iter()
            .filter(|status| status.connected && status.supported)
            .count();
        let feed_items =
            crate::core::connectivity::integration_sync::list_feed_items(integration_ctx, None, 18)
                .await;
...
pub(crate) async fn run_sentinel_scan_tick(
    ...
    let (storage, integration_ctx) = {
        ...
        (
            agent.storage.clone(),
            crate::core::connectivity::integration_sync::context_from_agent(&agent, None),
        )
    };
    ...
    let daily_review_notice_sent =
        should_send_standalone_sentinel_daily_review_notice(&settings, scan_state.open_proposals);
...
pub(super) async fn get_sentinel_feed(State(state): State<AppState>) -> Response {
    ...
    let feed_items =
        crate::core::connectivity::integration_sync::list_feed_items(&ctx, None, 18).await;
...
mod tests {
    ...
    #[test]
    fn standalone_sentinel_daily_review_notices_are_disabled() {
        let settings = AutonomySettings::default();

        assert!(!should_send_standalone_sentinel_daily_review_notice(
            &settings, 0
        ));
        assert!(!should_send_standalone_sentinel_daily_review_notice(
            &settings, 3
