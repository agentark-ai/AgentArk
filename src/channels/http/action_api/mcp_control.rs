...
pub(super) async fn build_mcp_config(
    ...
    crate::core::runtime::config::McpServerConfig,
    ...
    let transport = match &request.transport {
        McpTransportRequest::Http { url } => {
            ...
            crate::core::runtime::config::McpTransportConfig::Http { url: url.clone() }
        }
        McpTransportRequest::Stdio {
            ...
            let env_keys = env
                ...
                .unwrap_or_else(|| match existing.map(|cfg| &cfg.transport) {
                    Some(crate::core::runtime::config::McpTransportConfig::Stdio {
                        env_keys,
                        ..
                    }) => env_keys.clone(),
                    _ => Vec::new(),
    ...
    if let Some(profile_id) = auth_profile_id.as_deref() {
        if !matches!(
            &transport,
            crate::core::runtime::config::McpTransportConfig::Http { .. }
        ) {
    ...
    let config = crate::core::runtime::config::McpServerConfig {
...
pub(super) fn parse_mcp_auth(
    ...
    existing: Option<&crate::core::runtime::config::McpAuthConfig>,
) -> (
    ...
    match req {
        ...
        McpAuthRequest::Bearer {
            ...
            (
                Some(crate::core::runtime::config::McpAuthConfig::Bearer { header }),
                Some(McpAuthUpdate {
                    clear: *clear,
                    token: token.clone(),
                    username: None,
        ...
        McpAuthRequest::Basic {
            ...
            Some(crate::core::runtime::config::McpAuthConfig::Basic),
            Some(McpAuthUpdate {
                clear: *clear,
                token: None,
        ...
        McpAuthRequest::Header { name, value, clear } => (
            Some(crate::core::runtime::config::McpAuthConfig::Header { name: name.clone() }),
            Some(McpAuthUpdate {
                clear: *clear,
                token: value.clone(),
                username: None,
        ...
        McpAuthRequest::Query { name, value, clear } => (
            Some(crate::core::runtime::config::McpAuthConfig::Query { name: name.clone() }),
            Some(McpAuthUpdate {
                clear: *clear,
                token: value.clone(),
                username: None,
...
pub(super) fn apply_auth_update(
    secrets: &mut crate::core::runtime::config::Secrets,
    server_id: &str,
...
pub(super) fn load_mcp_secrets(agent: &Agent) -> Result<crate::core::runtime::config::Secrets> {
    let manager = crate::core::runtime::config::SecureConfigManager::new_with_data_dir(
        &agent.config_dir,
        Some(&agent.data_dir),
    )?;
...
pub(super) async fn sync_mcp_registry(
    agent: &Agent,
