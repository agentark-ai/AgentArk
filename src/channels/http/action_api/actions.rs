...
fn builtin_env_from_agent_config(
    cfg: &crate::core::runtime::config::AgentConfig,
    env: &str,
) -> bool {
...
fn env_is_configured_for_action(
    agent_cfg: &crate::core::runtime::config::AgentConfig,
    custom: &std::collections::HashMap<String, String>,
    action_name: &str,
    env: &str,
) -> bool {
    ...
    if crate::core::runtime::secrets::has_user_secret(custom, target) {
        return true;
    }
...
pub(super) async fn get_action_secrets(
    ...
    let mgr = match crate::core::runtime::config::SecureConfigManager::new_with_data_dir(
        &config_dir,
        Some(&data_dir),
    ) {
        Ok(m) => m,
...
pub(super) async fn create_action(
    ...
    let (missing_env, bindings) = if !required_env.is_empty() {
        let secrets = match crate::core::runtime::config::SecureConfigManager::new_with_data_dir(
            &agent_guard.config_dir,
...
async fn validate_import_fetch_url(raw: &str) -> Result<reqwest::Url, String> {
    crate::core::runtime::net::validate_public_https_url(raw)
        .await
...
pub(crate) async fn import_action_from_content_with_agent(
    ...
    let required_env = extract_frontmatter_text(&content)
        ...
        .unwrap_or_default();
    let (missing_env, bindings) = if !required_env.is_empty() {
        let secrets = crate::core::runtime::config::SecureConfigManager::new_with_data_dir(
            &agent.config_dir,
            Some(&agent.data_dir),
        )
        .and_then(|mgr| mgr.load_secrets())
