...
pub(super) async fn sync_http_api_key_state(
    state: &AppState,
    force_refresh: bool,
) -> std::result::Result<(Option<crate::core::runtime::config::HttpApiKeyInfo>, bool), String> {
    let now = unix_now_ts();
    if !force_refresh {
        ...
        if let (Some(key), Some(expires_at)) = (cached_key, cached_exp) {
            if expires_at > now {
                return Ok((
                    Some(crate::core::runtime::config::HttpApiKeyInfo {
                        key,
                        issued_at: now,
                        expires_at,
                    }),
    ...
    if info.is_none()
        && state.deployment_mode != crate::core::runtime::config::DeploymentMode::TrustedLocal
    {
...
fn valid_ui_session_request_context(
    headers: &HeaderMap,
    addr: SocketAddr,
    deployment_mode: crate::core::runtime::config::DeploymentMode,
) -> bool {
    if deployment_mode == crate::core::runtime::config::DeploymentMode::TrustedLocal {
...
pub(super) async fn auth_middleware(
    ...
    let Some(expected_key) = expected_key else {
        if state.deployment_mode == crate::core::runtime::config::DeploymentMode::TrustedLocal
