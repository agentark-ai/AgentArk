...
pub(super) fn release_update_apply_support(state: &AppState) -> (bool, Option<String>) {
    ...
    if !crate::core::runtime::release_updates::ui_update_supported_image(
        &crate::core::runtime::runtime_image::default_runtime_image(),
    ) {
        return (
            false,
            Some(
...
pub(super) async fn fetch_release_update_summary_uncached() -> UpdateStatusResponse {
    ...
    let latest = match crate::core::runtime::release_updates::fetch_latest_release_info(
        &client,
        &crate::core::runtime::release_updates::release_repo_slug(),
    )
    .await
    {
        Ok(latest) => latest,
    ...
    let state = match crate::core::runtime::release_updates::is_release_version_newer(
...
pub(super) async fn current_release_update_summary(state: &AppState) -> UpdateStatusResponse {
    ...
    // Refresh in the background and serve the last-known summary immediately.
    // /status is polled continuously by the UI and the outbound release
    // lookup takes seconds — no request should ever block on it. Pollers
    // pick up the refreshed value on their next tick (the very first call
    // serves the default "checking" summary).
    let stale_summary = {
        ...
        if !fresh && !cache.refreshing {
            ...
            let cache_handle = Arc::clone(&state.release_update_cache);
            tokio::spawn(async move {
                let fetched = fetch_release_update_summary_uncached().await;
                let mut cache = cache_handle.write().await;
                cache.last_checked_at = Some(Instant::now());
                cache.summary = fetched;
                cache.refreshing = false;
            });
        }
        cache.summary.clone()
    };
    stale_summary.with_apply_support(apply_supported, apply_message)
}
...
pub(super) struct EvolutionSettingsResponse {
    ...
    pub(super) prompt_fragment_canary: EvolutionCanarySummary,
    pub(super) last_promotion_result: String,
    pub(super) replay_gate_result: Option<String>,
    pub(super) replay_gate_reasons: Vec<crate::core::self_evolve::PromotionGateReason>,
    ...
    pub(super) prompt_replay_gate_reasons: Vec<crate::core::self_evolve::PromotionGateReason>,
    ...
    pub(super) specialist_prompt_replay_gate_reasons:
        Vec<crate::core::self_evolve::PromotionGateReason>,
    pub(super) specialist_prompt_promotion_mode: String,
    pub(super) prompt_fragment_last_promotion_result: String,
    pub(super) prompt_fragment_replay_gate_result: Option<String>,
    pub(super) prompt_fragment_replay_gate_reasons:
        Vec<crate::core::self_evolve::PromotionGateReason>,
...
pub(super) struct PromptOptimizationHoldoutCase {
    ...
    pub(super) matching_samples: usize,
...
pub(super) struct PromptOptimizationLifecycleState {
    ...
    #[serde(default)]
    pub(super) terminal_baseline_job_id: Option<String>,
    #[serde(default)]
...
pub(super) struct EvolutionDevResponse {
    ...
    pub(super) prompt_fragment_canary_state:
        Option<crate::core::self_evolve::strategy_runtime::CanaryRolloutState>,
    pub(super) prompt_fragment_last_result: Option<serde_json::Value>,
    pub(super) prompt_fragment_lineage_recent: Vec<serde_json::Value>,
    pub(super) prompt_fragment_metrics: Vec<PromptEvolutionMetric>,
    pub(super) prompt_fragment_insights: PromptEvolutionInsights,
    pub(super) learning_queue: crate::storage::LearningQueueCounts,
    ...
    pub(super) recent_prompt_fragment_runs: Vec<serde_json::Value>,
