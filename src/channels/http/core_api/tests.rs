use super::*;
use crate::core::automation::autonomy::{RecommendedAction, RiskEnvelope};
...
#[test]
fn background_session_detach_unlinks_watchers_instead_of_deleting_them() {
    let source = include_str!("../automation_api/automation_control.rs");
    let detach_body = source
        .split("pub(super) async fn detach_background_session_work")
        .nth(1)
        .and_then(|tail| {
            tail.split("pub(super) async fn get_background_session")
                .next()
        })
        .expect("detach handler body should be present");

    assert!(detach_body.contains("rebind_watcher_background_session(&state, watcher_id, None)"));
    assert!(!detach_body.contains("delete_watcher_entity(&state, watcher_id)"));
    assert!(detach_body.contains("\"deleted_watcher_ids\": Vec::<String>::new()"));
}

#[test]
fn background_session_delete_unlinks_watchers_instead_of_deleting_them() {
    let source = include_str!("../automation_api/automation_control.rs");
    let delete_body = source
        .split("pub(super) async fn delete_background_session")
        .nth(1)
        .and_then(|tail| {
            tail.split("pub(super) async fn list_automation_objects")
                .next()
        })
        .expect("delete handler body should be present");

    assert!(delete_body.contains("rebind_watcher_background_session(&state, watcher_id, None)"));
    assert!(!delete_body.contains("delete_watcher_entity(&state, watcher_id)"));
}

#[test]
fn agent_background_session_delete_unlinks_durable_work_instead_of_deleting_it() {
    let source = include_str!("../../../core/agent/operations/background_sessions.rs");
    let delete_branch = source
        .split("\"delete\" =>")
        .nth(1)
        .and_then(|tail| tail.split("\"update_delivery\" =>").next())
        .expect("agent background session delete branch should be present");

    assert!(delete_branch.contains("unlink_linked_background_tasks"));
    assert!(delete_branch.contains("set_background_session_id(id, None)"));
    assert!(!delete_branch.contains("delete_linked_background_tasks"));
    assert!(!delete_branch.contains("watcher_manager.delete(id)"));
}
...
#[test]
fn summarize_watch_result_does_not_claim_creation_without_watcher_id() {
    let action = test_recommended_action("watch", "Monitor public updates");
    let summary = summarize_autonomy_action_result(
        &action,
        &serde_json::json!({
            "status": "executed",
            "kind": "watch",
            "watcher_id": serde_json::Value::Null,
            "message": "Watcher creation did not return a durable watcher id."
        }),
    );

    assert!(!summary.contains("Created"));
    assert!(summary.contains("not created"));
    assert!(summary.contains("durable watcher id"));
}
...
fn arkpulse_managed_app_operation_remediation_maps_to_fix_plan() {
    ...
    match plan {
        Some(PulseFixPlan::ManagedAppOperation { app_id, operation }) => {
...
fn arkpulse_readonly_investigation_remediation_maps_to_fix_plan() {
    ...
    match plan {
        Some(PulseFixPlan::ReadonlyInvestigation { topic }) => {
            assert_eq!(
                topic,
...
#[tokio::test]
async fn chat_credential_prompt_dismiss_endpoint_clears_pending_prompt() {
    let (state, _config_dir, _data_dir) = build_test_state().await;
    let conversation_id = uuid::Uuid::new_v4().to_string();
    {
        let agent = state.agent.read().await;
    ...
    let response = router
        .oneshot(
            Request::builder()
                .method("DELETE")
                .uri(format!(
                    "/chat/credential-prompt?conversation_id={}",
                    conversation_id
                ))
                .body(Body::empty())
                .unwrap(),
    ...
    assert_eq!(status, StatusCode::OK, "unexpected response: {}", body);
    assert_eq!(
        body.get("present").and_then(|value| value.as_bool()),
        Some(false)
    );
    let agent = state.agent.read().await;
    assert!(agent
...
#[test]
fn prompt_optimization_holdout_cases_collapse_duplicate_validation_footprints() {
    let traces = (0..5)
        .map(|idx| {
            test_prompt_trace(
                &format!("trace-duplicate-footprint-{idx}"),
                4_200,
                12_000,
                0.018,
                true,
    ...
    let profile = profiles
        .iter()
        .find(|item| item.section == "action_catalog")
        .expect("action catalog section should have a profile");

    assert_eq!(profile.samples, 5);
    assert_eq!(profile.holdout_cases.len(), 1);
    assert_eq!(profile.holdout_cases[0].matching_samples, 5);
}
...
#[test]
fn retrying_terminal_prompt_optimization_resets_sample_baseline() {
    assert_eq!(
        prompt_optimization_next_sample_baseline(20, "candidate_rejected", 204, true),
        204
    );
    assert_eq!(
        prompt_optimization_next_sample_baseline(20, "blocked", 204, true),
        204
    );
    assert_eq!(
        prompt_optimization_next_sample_baseline(20, "candidate_rejected", 204, false),
        20
    );
    assert_eq!(
        prompt_optimization_next_sample_baseline(0, "suggested", 204, true),
        204
    );
    let rejected = PromptOptimizationLifecycleState {
        status: "candidate_rejected".to_string(),
    ...
    assert_eq!(
        prompt_optimization_terminal_retry_sample_gap(&rejected, 210),
        Some((6, 24))
    );
    assert_eq!(
        prompt_optimization_terminal_retry_sample_gap(&rejected, 228),
        None
    );
}

#[test]
fn terminal_prompt_optimization_without_baseline_marker_is_reconciled() {
    let old_rejected = PromptOptimizationLifecycleState {
        status: "candidate_rejected".to_string(),
        job_status: Some("completed".to_string()),
        queued_job_id: Some("gepa-job-old".to_string()),
        sample_baseline: 20,
        ..PromptOptimizationLifecycleState::default()
    };
    assert!(!prompt_optimization_terminal_lifecycle_is_recorded(
        &old_rejected,
        "candidate_rejected",
        "completed",
        Some("gepa-job-old"),
    ...
    let reconciled = PromptOptimizationLifecycleState {
        terminal_baseline_job_id: Some("gepa-job-old".to_string()),
        ..old_rejected
    };
    assert!(prompt_optimization_terminal_lifecycle_is_recorded(
        &reconciled,
...
#[test]
fn arkdistill_analytics_parses_wrapped_numeric_telemetry_payloads() {
    let rows = vec![crate::storage::entities::operational_log::Model {
        id: "arkdistill-wrapped".to_string(),
        created_at: "2026-05-28T10:00:00Z".to_string(),
        trace_id: None,
        conversation_id: None,
        channel: "chat".to_string(),
            ...
            serde_json::json!({
                "event": {
                    "telemetry": {
                        ...
                        "distilled_chars": "4000",
                        "saved_chars": "12000",
                        "estimated_saved_tokens": "3000",
                        "estimated_prompt_cost_saved_usd": "0.006"
                    }
                }
            })
            .to_string(),
    ...
    assert_eq!(summary.totals.result_count, 1);
    assert_eq!(summary.totals.original_chars, 16000);
    assert_eq!(summary.totals.distilled_chars, 4000);
    assert_eq!(summary.totals.estimated_saved_tokens, 3000);
    assert_eq!(summary.totals.savings_percent, 75.0);
    assert_eq!(summary.totals.estimated_prompt_cost_saved_usd, Some(0.006));
    assert_eq!(summary.by_tool[0].tool_name, "browse");
}

#[test]
fn arkdistill_analytics_can_summarize_trace_telemetry_fallback() {
    let traces = vec![test_arkdistill_trace(
        "trace-arkdistill",
        "fetch",
        "page_fetch",
        12000,
    ...
    let summary = summarize_arkdistill_traces_window_with_pricing(
        &traces,
        None,
        None,
        "hour",
...
#[test]
fn arkdistill_trace_telemetry_uses_step_timestamps_for_series_buckets() {
    let step_at = |hour: u32, saved_tokens: usize| crate::core::ExecutionStep {
        icon: "[distill]".to_string(),
        title: "ArkDistill Context Savings".to_string(),
        detail: "Recorded ArkDistill context savings".to_string(),
        step_type: "info".to_string(),
        data: Some(
            serde_json::json!({
                ...
                "action": "page_fetch",
                "original_chars": 4000,
                "distilled_chars": 2000,
                "saved_chars": 2000,
                "estimated_original_tokens": 1000,
                "estimated_distilled_tokens": 500,
                "estimated_saved_tokens": saved_tokens,
            })
            .to_string(),
        ),
        timestamp: chrono::DateTime::parse_from_rfc3339(&format!("2026-01-01T{hour:02}:15:00Z"))
            .unwrap()
            .with_timezone(&chrono::Utc),
        duration_ms: Some(0),
    };
    let steps = vec![step_at(10, 500), step_at(11, 700)];
    let traces = vec![crate::storage::ExecutionTraceSummaryRow {
        id: "trace-step-buckets".to_string(),
        message: "trace step buckets".to_string(),
        channel: "chat".to_string(),
    ...
    assert_eq!(summary.totals.estimated_saved_tokens, 1200);
    assert_eq!(
        summary
            .series
            .iter()
            .map(|point| (point.bucket_start.clone(), point.estimated_saved_tokens))
            .collect::<Vec<_>>(),
        vec![
            ("2026-01-01T10:00:00+00:00".to_string(), 500),
            ("2026-01-01T11:00:00+00:00".to_string(), 700),
        ]
    );
}

#[test]
fn arkdistill_analytics_merges_trace_telemetry_when_operational_rows_have_no_savings() {
    let rows = vec![crate::storage::entities::operational_log::Model {
        id: "arkdistill-zero-log".to_string(),
        created_at: "2026-01-01T00:00:00Z".to_string(),
        trace_id: None,
        conversation_id: None,
        channel: "chat".to_string(),
    ...
    let summary = summarize_arkdistill_logs_and_traces_window_with_pricing(
        &rows,
        &traces,
        None,
        None,
    ...
    assert_eq!(summary.series[0].estimated_saved_tokens, 2250);
}
