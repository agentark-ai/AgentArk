...
fn chat_stream_forwarder_tail_flush_timeout_ms() -> u64 {
    std::env::var("AGENTARK_CHAT_STREAM_FORWARDER_TAIL_FLUSH_MS")
        .ok()
        .and_then(|value| value.trim().parse::<u64>().ok())
...
async fn finish_chat_stream_auxiliary_tasks(
    trace_poller: tokio::task::JoinHandle<()>,
    stream_forwarder: tokio::task::JoinHandle<()>,
) {
    finish_chat_stream_auxiliary_tasks_with_timeout(
        trace_poller,
        stream_forwarder,
        Duration::from_millis(chat_stream_forwarder_tail_flush_timeout_ms()),
    )
    .await;
}

async fn finish_chat_stream_auxiliary_tasks_with_timeout(
    trace_poller: tokio::task::JoinHandle<()>,
    mut stream_forwarder: tokio::task::JoinHandle<()>,
    forwarder_timeout: Duration,
) {
    trace_poller.abort();
    let _ = trace_poller.await;

    if tokio::time::timeout(forwarder_timeout, &mut stream_forwarder)
        .await
        .is_err()
    {
        tracing::debug!(
            timeout_ms = forwarder_timeout.as_millis() as u64,
            "Chat stream forwarder did not close before terminal event flush; aborting auxiliary drain"
        );
...
where
    ...
    F::Output: crate::core::platform::spawn::SpawnLoggedOutcome + Send + 'static,
{
    let runtime = chat_worker_runtime()?;
    runtime.spawn(async move {
        let permits = chat_worker_permits();
        ...
        match std::panic::AssertUnwindSafe(future).catch_unwind().await {
            Ok(output) => {
                crate::core::platform::spawn::SpawnLoggedOutcome::log_if_error(output, task_name);
            }
            Err(payload) => {
                tracing::error!(
                    task = task_name,
...
fn attach_chat_request_context_to_hints(
    ...
    client_timezone_offset_minutes: Option<i32>,
) -> crate::core::RequestExecutionHints {
    ...
    hints.client_timezone = client_timezone;
    hints.client_timezone_offset_minutes = client_timezone_offset_minutes;
    hints
}
...
pub(super) async fn chat(
    ...
    if let Some((key, value)) = parse_set_secret_command(&request.message) {
        if !crate::core::runtime::secrets::setsecret_command_escape_hatch_enabled() {
            return (
                StatusCode::OK,
                Json(ChatResponse {
                    response: crate::core::runtime::secrets::setsecret_command_disabled_response()
                        .to_string(),
                    proof_id: None,
                    conversation_id: request.conversation_id.clone(),
                    conversation_title: None,
    ...
    if let Some(key) =
        crate::core::runtime::secrets::parse_use_current_llm_key_command(&request.message)
    {
        if !crate::core::runtime::secrets::secret_command_escape_hatch_enabled() {
            return (
                StatusCode::OK,
                Json(ChatResponse {
                    response: crate::core::runtime::secrets::setsecret_command_disabled_response()
        ...
        if let Err(e) = crate::core::runtime::secrets::store_user_secret(
            &config_dir,
            Some(&data_dir),
            &key,
            &value,
    ...
    let client_timezone = request.client_timezone.clone();
    let client_timezone_offset_minutes = request.client_timezone_offset_minutes;
    ...
    match result {
        Ok(processed) => (
            StatusCode::OK,
            Json(ChatResponse {
                response: crate::core::model::llm_context_sanitizer::strip_internal_tool_transcript(
                    &processed.response,
                ),
                proof_id: None,
...
pub(super) fn normalize_stream_event_for_sse(
    ...
    match ev {
        ...
        crate::core::StreamEvent::ReasoningDelta {
            ...
            let sanitized_delta =
                crate::core::model::llm_context_sanitizer::strip_internal_tool_transcript_preserve_spacing(
...
fn foreground_chat_execution_profile(
    requested: Option<ChatExecutionProfile>,
    legacy_deep_research: bool,
) -> Option<ChatExecutionProfile> {
    if legacy_deep_research {
        Some(ChatExecutionProfile::deep_research_override_with(requested))
    } else {
        requested
    }
}
...
pub(super) struct ChatStreamRunRequest {
    ...
    pub(super) client_timezone: Option<String>,
    pub(super) client_timezone_offset_minutes: Option<i32>,
...
pub(super) fn chat_stream_run_request_from_persisted_user_message(
    ...
    ChatStreamRunRequest {
        ...
        client_timezone_offset_minutes: request.client_timezone_offset_minutes,
        caller_principal,
...
async fn send_chat_stream_done(
    tx: &tokio::sync::mpsc::Sender<std::result::Result<Event, std::convert::Infallible>>,
    live_runs: &std::sync::Arc<crate::core::LiveRunRegistry>,
    run_id: &str,
    origin: &str,
    ...
    if let Err(error) = spawn_chat_worker_logged("http_chat_stream_turn", async move {
        ...
        let stream_forwarder = {
            ...
            crate::spawn_logged!("src/channels/http.rs:14840", async move {
                ...
                // Token-forwarding telemetry: proves whether model text
                // deltas left the server, and how many the transcript filter
                // withheld, so a "no visible model prose" report can be
                // attributed to the backend or the browser from logs alone.
                let mut forwarded_token_events: u64 = 0;
                let mut forwarded_token_chars: u64 = 0;
                let mut filtered_token_events: u64 = 0;
                while let Some(ev) = stream_rx.recv().await {
                    let ev = match ev {
                        crate::core::StreamEvent::Token(content) => {
                            let content = internal_token_filter.feed(&content);
                            if content.is_empty() {
                                filtered_token_events += 1;
                                continue;
                            }
                            forwarded_token_events += 1;
                            forwarded_token_chars += content.chars().count() as u64;
                            crate::core::StreamEvent::Token(content)
                        }
                        other => other,
                    };
                ...
                let held_tail_chars = tail.chars().count() as u64;
                if !tail.is_empty() {
                    send_chat_stream_event(
                        &tx,
                        &live_runs,
                ...
                tracing::info!(
                    ...
                    forwarded_token_chars,
                    filtered_token_events,
                    held_tail_chars,
                    "Chat stream token forwarding summary"
                );
            })
        };
        ...
        if let Some(accepted) = accepted_suggestion.clone() {
                ...
                .eq_ignore_ascii_case("watcher")
            {
                ...
                .await;
                send_chat_stream_done(&tx, &live_runs, &stream_request_id, &channel).await;
                drop(stream_tx);
                finish_chat_stream_auxiliary_tasks(trace_poller, stream_forwarder).await;
                return;
            }
        }

        let execution_profile = if accepted_suggestion.is_none() && task_mode_create_if_needed {
            foreground_chat_execution_profile(
                requested_execution_profile.clone(),
                legacy_deep_research,
            )
        } else {
            requested_execution_profile.clone()
        };
        ...
        let mut process_handle = {
            ...
            let client_timezone = client_timezone.clone();
            let client_timezone_offset_minutes = client_timezone_offset_minutes;
        ...
        let mut stream_done_sent = false;
        match result {
            Ok(mut processed) => {
                processed.response =
                ...
                if let Some(accepted) = accepted_suggestion.clone() {
                    ...
                    let suggestion_app_state = app_state.clone();
                    let suggestion_run_storage = run_storage.clone();
                    let suggestion_summary = persist_summary.clone();
                    crate::spawn_logged!(
                        "src/channels/http/chat_control.rs:update_accepted_suggestion_after_stream_done",
                        async move {
                            let outcomes = suggestions::collect_run_outcomes(
                                &suggestion_app_state,
                                &accepted.before,
                                &accepted.suggestion.kind,
                            )
                            .await;
...
pub(super) async fn chat_stream(
    ...
    if let Some((key, value)) = parse_set_secret_command(&request.message) {
        if !crate::core::runtime::secrets::setsecret_command_escape_hatch_enabled() {
            let payload = serde_json::json!({
                "content": crate::core::runtime::secrets::setsecret_command_disabled_response(),
                "conversation_id": request.conversation_id,
            });
        ...
        crate::core::runtime::secrets::parse_use_current_llm_key_command(&request.message)
    {
        ...
        {
            match crate::core::runtime::secrets::store_user_secret(
                &config_dir,
                Some(&data_dir),
                &key,
                &value,
...
mod tests {
    ...
    fn chat_stream_request_uses_execution_profile_over_legacy_boolean() {
        ...
        assert!(profile
            ...
            .any(|value| value == "research"));
        assert!(profile.long_running);
        assert_eq!(profile.source, "ui_override");
    }

    #[test]
    fn chat_stream_request_preserves_client_timezone_context() {
        let request = ChatRequest {
            message: "Remind me at 10 PM today".to_string(),
            channel: "web".to_string(),
            conversation_id: None,
            deep_research: false,
            execution_profile: None,
        ...
        assert_eq!(run_request.client_timezone.as_deref(), Some("Asia/Kolkata"));
        assert_eq!(run_request.client_timezone_offset_minutes, Some(330));
    }
    ...
    #[test]
    fn foreground_chat_without_explicit_profile_uses_single_spine_path() {
        assert!(foreground_chat_execution_profile(None, false).is_none());
    }

    #[test]
    fn foreground_chat_preserves_explicit_profile_without_classifier() {
        let profile = ChatExecutionProfile::from_classifier_value(
            &serde_json::json!({
                "capability_tags": ["research"],
                "depth_hint": "deep",
        ...
        assert_eq!(
            foreground_chat_execution_profile(Some(profile.clone()), false),
            Some(profile)
        );
    }

    #[tokio::test]
    async fn chat_stream_auxiliary_finish_does_not_wait_for_slow_forwarder() {
        let trace_poller = tokio::spawn(async {
            tokio::time::sleep(std::time::Duration::from_secs(60)).await;
        });
        let stream_forwarder = tokio::spawn(async {
            tokio::time::sleep(std::time::Duration::from_secs(60)).await;
        });
    ...
    #[test]
    fn first_stream_activity_counts_console_reasoning_and_visible_work() {
        assert_eq!(
        ...
        assert_eq!(
            stream_event_first_activity_source(&crate::core::StreamEvent::ReasoningDelta {
                phase: "vendor-specific.deep-thought.phase".to_string(),
                content_delta: "Checking provider-specific reasoning.".to_string(),
                done: false,
            }),
            Some("reasoning_delta")
        );
    ...
    #[test]
    fn reasoning_delta_sse_streams_console_model_reasoning() {
        ...
        let (event_name, payload) = event.expect("model reasoning should stream to console");

        assert_eq!(event_name, "reasoning_delta");
        assert_ne!(event_name, "token");
    ...
    #[test]
    fn reasoning_delta_sse_streams_console_reasoning_done_marker() {
        let (event, _) = normalize_stream_event_for_sse(
            crate::core::StreamEvent::ReasoningDelta {
                phase: "model".to_string(),
                content_delta: String::new(),
        ...
        let (event_name, payload) = event.expect("reasoning done marker should stream to console");

        assert_eq!(event_name, "reasoning_delta");
        assert_eq!(payload["title"], "Thinking");
        assert_eq!(payload["detail"], "Thinking completed.");
        assert_eq!(payload["done"], true);
    }

    #[test]
    fn arbitrary_provider_reasoning_phase_streams_to_console() {
        let (event, _) = normalize_stream_event_for_sse(
            crate::core::StreamEvent::ReasoningDelta {
                phase: "vendor-specific.deep-thought.phase".to_string(),
        ...
        let (event_name, payload) = event.expect("reasoning summary should stream to console");

        assert_eq!(event_name, "reasoning_delta");
        assert_eq!(payload["title"], "Thinking");
        assert_eq!(payload["phase"], "vendor-specific.deep-thought.phase");
        assert_eq!(
            payload["content_delta"],
