...
pub(super) async fn llm_analytics_endpoint(
    ...
    let (arkdistill_logs, arkdistill_truncated) = agent
        .storage
        .list_operational_logs_by_event_window_complete(
            crate::core::ARKDISTILL_EVENT_TYPE,
            &since_rfc3339,
            &until_rfc3339,
        )
        .await
        .unwrap_or_default();
    let arkdistill_trace_rows = agent
        .storage
        .list_execution_trace_summaries(Some(&since_rfc3339), 20_000, 0)
        .await
        .unwrap_or_default()
        .into_iter()
        .filter(|trace| {
            parse_utc_rfc3339(&trace.created_at).is_some_and(|created_at| created_at <= until)
        })
        .collect::<Vec<_>>();
    drop(agent);
    ...
    let arkdistill_needs_openrouter_prices =
        arkdistill_logs_need_openrouter_pricing(&arkdistill_logs, &arkdistill_pricing_context)
            || arkdistill_traces_need_openrouter_pricing(
    ...
    let truncated = truncated || arkdistill_truncated;
