...
impl ResearchClient {
    ...
    async fn fetch_content(
        ...
        let validated_url = crate::core::runtime::net::validate_public_https_url(url).await?;
        if let Some(progress) = progress {
            progress.emit(
                "reading",
                "Reading sources",
    ...
    async fn fetch_html_with_safe_redirects(
        ...
        for _ in 0..=MAX_RESEARCH_FETCH_REDIRECTS {
            ...
            if response.status().is_redirection() {
                ...
                current =
                    crate::core::runtime::net::validate_public_https_url(next.as_str()).await?;
                continue;
            }
...
fn research_inline_chart_blocks(args: &ResearchArgs, result: &ResearchResult) -> String {
    ...
    if result.sources.len() >= 2 {
        let data = result
            ...
            .collect::<Vec<_>>();
        blocks.push(crate::core::platform::inline_artifacts::inline_chart_block(
            &serde_json::json!({
