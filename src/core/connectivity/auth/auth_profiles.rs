...
) -> Result<Option<crate::core::connectivity::browser_profiles::BrowserLoginState>> {
    let payload =
        crate::core::connectivity::browser_profiles::BrowserProfileControlPlane::list(storage)
            .await?;
    Ok(payload
        .profiles
        .into_iter()
...
async fn profile_http_status(
    ...
    match &profile.material {
        ...
        AuthProfileMaterial::BrowserSession {
            ...
            match login_state {
                Some(crate::core::connectivity::browser_profiles::BrowserLoginState::LoggedIn) => Ok((
                    AuthProfileStatus::NeedsConnect,
                    false,
                    Some(
                ...
                Some(crate::core::connectivity::browser_profiles::BrowserLoginState::NeedsMfa) => Ok((
                    AuthProfileStatus::NeedsConnect,
                    false,
                    Some("Browser profile still needs MFA before it can provide a session.".to_string()),
                )),
                Some(crate::core::connectivity::browser_profiles::BrowserLoginState::Expired) => Ok((
                    AuthProfileStatus::Expired,
                    false,
