...
pub(super) const SPINE_PROMPT_BUNDLE_VERSION: &str = "spine_prompt_bundle_v2";
...
pub(super) fn build_spine_prompt_bundle(
    ...
    fragment_profile: Option<&crate::core::model::prompt_fragments::PromptFragmentBundleProfile>,
    primitive_names: &[&str],
) -> SpinePromptBundle {
    ...
    SpinePromptBundle {
        stable_prefix: vec![
            ...
            stable_fragment(
                "spine.durable_resource_contracts",
                durable_resource_contracts_fragment(),
            ),
            stable_fragment(
...
fn tool_result_contract_fragment() -> &'static str {
    "Tools and results are source of truth. Do not invent tool results, IDs, links, schedules, managed resources, credentials, notifications, or filesystem paths. \
     Durable resource create, update, delete, lifecycle, schedule, and notification claims must be grounded in a successful tool result from the current turn; if no such result exists, do not say it was created, updated, rescheduled, cancelled, or sent. \
...
fn artifact_delivery_policy_fragment() -> &'static str {
    "When the user wants a browser-viewable page, HTML report, dashboard, app, game, tool, or other UI artifact, deliver it through app_deploy so the user gets an accessible /apps/ URL. \
     ...
     Use pdf_generate for PDF files from final content; do not create PDFs by running ad-hoc code and then copying temporary files through resource_rw. For research/report PDFs, preserve agentark-chart fences or numeric Markdown tables in the final content so the PDF generator can render a visual summary automatically. Use file_write for raw documents, runbooks, source assets, and non-runnable artifacts; set document_visible=true when the file is meant for the user's Documents surface, then refer to its human-readable label or Documents, not an internal filesystem path. \
...
fn background_automation_policy_fragment() -> &'static str {
    "When the user requests durable recurring automation or background execution, translate their intended cadence into resource_rw kind=scheduled_task or watcher as appropriate; they do not need to know AgentArk's internal names. \
     Follow the non-evolvable durable resource contract fragment for scheduled_task versus watcher ownership and schedule-field shape. Plain reminders and notification-only date/time requests are AgentArk scheduled tasks, not external calendar entries, unless the user explicitly asks to create or modify an external calendar event. Preserve the user's requested wall-clock time and timezone in the schedule arguments; use cron for recurring schedules, at/scheduled_for for fully known one-time timestamps, and structured local_time/timezone when only a wall-clock time is known so the scheduler resolves the date from runtime temporal context instead of relying on manual date arithmetic. If the exact time cannot be represented, fail or ask rather than rounding to now. \
...
fn memory_policy_fragment() -> &'static str {
    "Use memory_rw read/search only when saved memory is needed to answer the current request. \
     In user-facing chat, do not use foreground memory_rw write/update/delete; memory mutations from the user's source message are queued after chat persistence and ArkMemory processes them asynchronously. \
     You may acknowledge that memory will be handled in the background, but continue the main answer without waiting for memory capture. \
     Outside user-facing chat, use memory_rw write/update/delete only when the user's current intent is active memory management. \
