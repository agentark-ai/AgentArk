...
impl Agent {
    ...
    pub(super) async fn load_pending_integration_connect_flow(
        &self,
        conversation_id: &str,
    ) -> Option<crate::core::connectivity::connect_flow::PendingIntegrationConnect> {
        ...
        let flow = self
            .load_encrypted_json::<crate::core::connectivity::connect_flow::PendingIntegrationConnect>(&key)
            .await?;
    ...
    pub(super) async fn continue_integration_connect_flow_after_secret_save(
        ...
        if (now - flow.started_at).num_seconds()
            > crate::core::connectivity::connect_flow::CONNECT_FLOW_TTL_SECS
        {
        ...
        let spec = match crate::core::connectivity::connect_flow::spec_by_id(&flow.integration_id) {
            Some(s) => s,
            None => {
                self.clear_pending_integration_connect_flow(conversation_id)
                    .await;
        ...
        let secret_present = |user_key: &str| -> bool {
            for storage_key in crate::core::runtime::secrets::storage_keys_for_user_key(user_key) {
        ...
        let required_ok = match spec.required.kind {
            crate::core::connectivity::connect_flow::SecretRequirementKind::All => {
                spec.required.keys.iter().all(|k| secret_present(k))
            }
            crate::core::connectivity::connect_flow::SecretRequirementKind::Any => {
    ...
    pub(super) async fn submit_pending_integration_auth_credentials(
        ...
        use crate::core::connectivity::integration_auth::{AuthMode, IntegrationAuthManifest};
        ...
        for field in &form_fields {
            if let Some(value) = values.get(&field.key).map(|value| value.trim()) {
                ...
                for target in &field.storage_targets {
                    crate::core::runtime::secrets::store_user_secret(
                        &self.config_dir,
                        Some(&self.data_dir),
                        target,
                        value,
    ...
    pub(super) async fn submit_pending_raw_secret_credentials(
        ...
        use crate::core::connectivity::integration_auth::AuthMode;

        let manifest = crate::core::connectivity::integration_auth::raw_key_manifest(&key);
    ...
    pub(super) async fn submit_pending_mcp_server_auth_credentials(
        ...
        let mut secret = manager
            ...
            .unwrap_or_default();
        match auth {
            crate::core::runtime::config::McpAuthConfig::Bearer { .. }
            | crate::core::runtime::config::McpAuthConfig::Header { .. }
            | crate::core::runtime::config::McpAuthConfig::Query { .. } => {
                let token = values
                    .get("token")
    ...
    async fn mcp_server_auth_prompt_is_satisfied(&self, server_id: &str) -> bool {
        ...
        if let Some(profile_id) = server.auth_profile_id.as_deref() {
            return crate::core::connectivity::auth_profiles::AuthProfileControlPlane::get(
                &self.storage,
                profile_id,
            )
            .await
        ...
        match auth {
            crate::core::runtime::config::McpAuthConfig::Bearer { .. }
            | crate::core::runtime::config::McpAuthConfig::Header { .. }
            | crate::core::runtime::config::McpAuthConfig::Query { .. } => secret
                .token
                .as_deref()
    ...
    pub(super) async fn submit_pending_custom_api_auth_credentials(
        ...
        self.refresh_action_catalog_index("custom_api_auth_saved")
            .await;
        self.spawn_custom_api_auth_ready_reconcile(vec![api_id.clone()], "custom_api_auth_saved");

        self.clear_pending_chat_credential_prompt(conversation_id)
            .await;
        let mut response = format!(
            "Saved credentials for {} securely. The value was not sent to the assistant. AgentArk is refreshing the integration and checking it in the background.",
            api_name
        );
        if let Some(followup) = self.on_secret_saved_followup(conversation_id).await {
            response.push_str("\n\n");
    ...
    pub(crate) async fn lookup_integration_auth_manifest(
        &self,
        integration_id: &str,
    ) -> Option<crate::core::connectivity::integration_auth::IntegrationAuthManifest> {
        let needle = integration_id.trim();
        if needle.is_empty() {
            return None;
        }
        if needle == crate::core::connectivity::integration_auth::RAW_KEY_MANIFEST_ID {
        ...
        if let Ok(Some(pack_view)) = registry.get_pack(needle).await {
            if let Some(manifest) =
                crate::core::connectivity::integration_auth::manifest_from_extension_pack(
                    &pack_view.manifest,
                )
            {
    ...
    pub(crate) async fn integration_auth_manifests(
        &self,
    ) -> Vec<crate::core::connectivity::integration_auth::IntegrationAuthManifest> {
        let registry = self.extension_packs.read().await;
    ...
    /// Convert an [`crate::core::connectivity::integration_auth::IntegrationAuthManifest`]
    ...
    pub(super) fn chat_credential_prompt_from_manifest(
        manifest: &crate::core::connectivity::integration_auth::IntegrationAuthManifest,
    ) -> ChatCredentialPrompt {
        use crate::core::connectivity::integration_auth::{
            AuthMode, FieldInputType, RAW_KEY_MANIFEST_ID,
        };
    ...
    pub(super) fn integration_auth_settings_path(
        ...
        if integration_id == crate::core::connectivity::integration_auth::RAW_KEY_MANIFEST_ID {
            return None;
        }
    ...
    pub async fn pending_chat_credential_prompt(
            ...
            if Self::chat_credential_prompt_is_expired(
                ...
                match pending.kind {
                    ...
                    PendingChatCredentialPromptKind::RawSecret { key, origin } => {
                        ...
                        let manifest =
                            crate::core::connectivity::integration_auth::raw_key_manifest(&key);
            ...
            .await
        {
            if Self::chat_credential_prompt_is_expired(
                ...
                crate::core::connectivity::connect_flow::spec_by_id(&flow.integration_id)
            {
                let mut fields = spec
                    .required
                    .keys
                ...
                let description = match spec.required.kind {
                    crate::core::connectivity::connect_flow::SecretRequirementKind::All => format!(
                        "Finish connecting {} by entering the required secrets below. AgentArk will test and enable it after the values are saved.",
                        spec.name
                    ),
                    crate::core::connectivity::connect_flow::SecretRequirementKind::Any => format!(
            ...
            if let Some(pending) = self.load_pending_secret_followup(cid).await {
                if (chrono::Utc::now() - pending.requested_at) <= chrono::Duration::minutes(30) {
                        ...
                        pending.kind
                    {
                        ...
                        let mut response =
                            format!("Saved encrypted environment values for app '{}'.", title);
        ...
        match pending.kind {
            ...
            PendingSecretFollowupKind::RestartApp {
                ...
                let still_missing: Vec<String> = missing_env
                    ...
                    .filter(|key| {
                        let app_has_secret =
                            crate::actions::app::app_env_secret_storage_key(&app_id, key)
                                .and_then(|storage_key| custom.get(&storage_key))
                                .is_some_and(|value| !value.trim().is_empty());
                        !app_has_secret
                            && !crate::core::runtime::secrets::has_user_secret(custom, key)
