...
const SEMANTIC_WATCHER_POLL_ACTION: &str = "semantic_watcher_poll";

#[derive(Debug, Clone)]
struct SemanticWatcherPollContract {
    poll_action: String,
...
fn schedule_task_batch_item_arguments(
    ...
    // Collect every item violation in one pass so the model can repair the
    // whole batch in a single corrected call instead of one item per turn.
    let mut violations: Vec<String> = Vec::new();
    for (index, item) in items.iter().enumerate() {
        let Some(item_obj) = item.as_object() else {
            violations.push(format!("schedule_task.items[{}] must be an object", index));
            continue;
        };
        ...
        if !has_task_ref && !has_schedule {
            violations.push(format!(
                "schedule_task.items[{}] needs `task` (what to do) and a schedule (`cron`, `at`, `scheduled_for`, or `local_time`)",
                index
            ));
            continue;
        }
        if !has_task_ref {
            violations.push(format!(
                "schedule_task.items[{}] needs `task` (what to do) or `task_id`",
                index
            ));
            continue;
        }
        if !has_schedule {
            violations.push(format!(
                "schedule_task.items[{}] needs a schedule: `cron`, `at`, `scheduled_for`, or `local_time`",
                index
            ));
            continue;
        }
    ...
    if !violations.is_empty() {
        return Some(Err(anyhow::anyhow!("{}", violations.join("; "))));
    }
    Some(Ok(out))
}

fn schedule_task_batch_violation_messages(detail: &str) -> Vec<String> {
    detail
        .split("; ")
        .map(str::trim)
...
fn watcher_action_is_semantic_poll(action_name: &str) -> bool {
    action_name
        .trim()
        .eq_ignore_ascii_case(SEMANTIC_WATCHER_POLL_ACTION)
}

fn watcher_semantic_source_payload(arguments: &serde_json::Value) -> serde_json::Value {
    ...
    for (key, value) in object {
        if !control_fields.iter().any(|field| field == key) {
...
fn semantic_watcher_poll_contract_from_arguments(
    arguments: &serde_json::Value,
    description: &str,
    condition: &crate::core::automation::watcher::WatchCondition,
    on_trigger: &str,
) -> Option<SemanticWatcherPollContract> {
    let description = description.trim();
    if description.is_empty() || description.eq_ignore_ascii_case("background watcher") {
        return None;
    }
    let condition_value = serde_json::to_value(condition).ok()?;
    Some(SemanticWatcherPollContract {
        poll_action: SEMANTIC_WATCHER_POLL_ACTION.to_string(),
        poll_arguments: serde_json::json!({
            ...
            "intent": {
                ...
                "on_trigger": on_trigger.trim(),
            },
            "source_payload": watcher_semantic_source_payload(arguments),
            "execution_contract": {
                "phase": "poll",
                ...
                "return_contract": "Return final text as one JSON object using execution_contract.result_contract. Set durable_observation_result=true and ready_for_change_detection=true only when every target has a stable source, comparable=true, non-empty observed_values, and empty blocking_gaps. If comparable observations are missing, return the same shape with ready_for_change_detection=false and blocking_gaps explaining what prevented a commit-ready observation. Do not notify the user or evaluate the trigger condition during the poll."
            }
        }),
    })
}
...
fn watch_batch_item_arguments(
    ...
    for (index, item) in items.iter().enumerate() {
        ...
        let has_script = merged
            ...
            .map(str::trim)
            .is_some_and(|value| !value.is_empty());
        let has_semantic_poll_contract = merged
            .get("description")
            .and_then(|value| value.as_str())
            .map(str::trim)
            .is_some_and(|value| !value.is_empty())
        ...
        let can_create = merged
            ...
            .map(str::trim)
            .is_some_and(|value| !value.is_empty())
            && (has_poll_action || has_script || has_semantic_poll_contract)
            && merged.get("condition").is_some()
            && merged
                .get("on_trigger")
                .and_then(|value| value.as_str())
...
fn schedule_task_default_timezone_from_profile_or_request(
    ...
    profile_timezone
        ...
        .or_else(|| {
            authorization
                .and_then(|context| context.request_timezone.as_deref())
                .map(str::trim)
                .filter(|value| !value.is_empty())
                .and_then(|value| value.parse::<chrono_tz::Tz>().ok())
        })
...
fn normalize_invalid_scheduled_notification_action(
    ...
    let explicit_action = explicit_action
        .map(str::trim)
        .filter(|value| !value.is_empty())?;
    if all_actions
        .iter()
        .any(|action| action.name == explicit_action)
    {
        return Some(explicit_action.to_string());
    }
    if !all_actions
        .iter()
    ...
    if report_to.trim().is_empty() || !scheduled_task_args_are_notification_only(task_args) {
        return None;
    }
    if let Some(object) = task_args.as_object_mut() {
    ...
    *task_args = build_notify_user_action_arguments(task_args, task_desc, report_to);
    Some("notify_user".to_string())
}

fn scheduled_task_args_are_notification_only(task_args: &serde_json::Value) -> bool {
    let Some(object) = task_args.as_object() else {
        return false;
    };
    object.keys().all(|key| {
        matches!(
...
fn parse_schedule_local_time(value: &str) -> Result<chrono::NaiveTime, String> {
    ...
    if let Some(time) = parse_schedule_flexible_clock_time(&normalized) {
        return Ok(time);
    }
    Err(format!(
...
fn parse_schedule_flexible_clock_time(value: &str) -> Option<chrono::NaiveTime> {
    static CLOCK_RE: std::sync::OnceLock<regex::Regex> = std::sync::OnceLock::new();
    let regex = CLOCK_RE.get_or_init(|| {
        regex::Regex::new(
            r#"(?i)^\s*(?P<hour>\d{1,2})(?:\s*(?:[:"'.]|h(?:rs?|ours?)?|\s+)\s*(?P<minute>\d{1,2}))?(?:\s*(?:[:"'.]|m(?:ins?|inutes?)?|\s+)\s*(?P<second>\d{1,2}))?\s*(?P<suffix>[a-z.\s]*)\s*$"#,
        )
        .expect("valid schedule local_time clock regex")
    });
    ...
    let suffix = captures
        ...
        .unwrap_or("");
    let period = classify_schedule_clock_suffix(suffix)?;
    let hour = resolve_schedule_clock_hour(hour, period)?;
    chrono::NaiveTime::from_hms_opt(hour, minute, second)
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum ScheduleClockPeriod {
    None,
    Am,
    Pm,
    Morning,
...
fn classify_schedule_clock_suffix(value: &str) -> Option<ScheduleClockPeriod> {
    let compact = value
        .chars()
    ...
    let compact = compact
        ...
        .trim();
    match compact {
        "" | "h" | "hr" | "hrs" | "hour" | "hours" => Some(ScheduleClockPeriod::None),
        "am" => Some(ScheduleClockPeriod::Am),
        "pm" => Some(ScheduleClockPeriod::Pm),
        "morning" => Some(ScheduleClockPeriod::Morning),
        "afternoon" | "evening" => Some(ScheduleClockPeriod::AfternoonOrEvening),
        "night" | "tonight" => Some(ScheduleClockPeriod::Night),
        _ => None,
    }
}

fn resolve_schedule_clock_hour(hour: u32, period: ScheduleClockPeriod) -> Option<u32> {
    match period {
        ScheduleClockPeriod::None => (hour <= 23).then_some(hour),
        ScheduleClockPeriod::Am => match hour {
            1..=11 => Some(hour),
            12 => Some(0),
            _ => None,
        },
        ScheduleClockPeriod::Pm => match hour {
            1..=11 => Some(hour + 12),
            12 => Some(12),
            _ => None,
        },
        ScheduleClockPeriod::Morning => match hour {
            0..=11 => Some(hour),
            12 => Some(0),
            _ => None,
        },
        ScheduleClockPeriod::AfternoonOrEvening => match hour {
            1..=11 => Some(hour + 12),
            12..=23 => Some(hour),
            _ => None,
        },
        ScheduleClockPeriod::Night => match hour {
            0 => Some(0),
            1..=11 => Some(hour + 12),
            12 => Some(0),
            13..=23 => Some(hour),
...
/// Single-source argument contract for schedule_task create/update. Surfaced
/// to the spine model in the resource_rw schema, embedded in every validation
/// failure, and pinned by parity tests so the taught contract and the
/// validator can never drift.
pub(crate) fn schedule_task_expected_contract() -> serde_json::Value {
    serde_json::json!({
        "required": "`task` (string: what to do) plus exactly one schedule source: `cron` (5-field, minute granularity, recurring) | `at`/`scheduled_for` (ISO 8601 one-time) | `local_time` (wall-clock, with optional `local_date` and `timezone`). `task_id` may replace `task` when updating an existing task.",
        "ownership": "Use schedule_task for pure time-based execution: reminders, known one-time work, or recurring jobs whose action/script is explicitly supplied. If delivery depends on observed state, a condition, a material difference, or change-only behavior, use watch instead.",
        "execution": "If `action`/`action_arguments` or `script` is omitted, the task is a notification/reminder delivered at the schedule. Autonomous checks or external work must include an explicit enabled `action` with `action_arguments`, or a `script`.",
        "batch": "For multiple independent outcomes use `items`: a non-empty array of objects, each satisfying the same task+schedule contract. Top-level fields (report_to, action, action_arguments, script, validation, ...) are inherited by items unless overridden.",
        "optional": [
            "report_to", "action", "action_arguments", "script", "script_language",
            "context_from", "workdir", "validation", "max_attempts",
            "stall_timeout_secs", "retry_backoff_secs", "allow_duplicate",
            "timezone", "timezone_offset_minutes", "date_policy"
        ],
        "example": {
            "task": "Send the daily standup reminder.",
            "cron": "0 9 * * 1-5",
            "action": "notify_user",
            "action_arguments": {
        ...
        "batch_example": {
            "items": [
                {
                    "task": "Send the morning operations reminder.",
                    "cron": "0 10 * * *",
                    "action": "notify_user",
                    "action_arguments": { "message": "Review the morning operations checklist." }
                },
                {
                    "task": "Send the evening handoff reminder.",
                    "cron": "0 22 * * *",
                    "action": "notify_user",
                    "action_arguments": { "message": "Prepare the evening handoff notes." }
                }
            ]
        }
    })
...
/// Single-source argument contract for watch create/update. Same role as
/// [`schedule_task_expected_contract`].
pub(crate) fn watch_expected_contract() -> serde_json::Value {
    serde_json::json!({
        "ownership": "Use watch for autonomous monitoring where delivery depends on observed state: conditions, changes, material differences, trigger matches, or notify-only-when behavior. A cadence such as interval_secs controls polling but does not make this a schedule_task.",
        "shape": "A monitored target - semantic `description` of what to observe (preferred; AgentArk polls it model-routed), or explicit `poll_action`+`poll_arguments`, or `script` - plus a structured `condition` object: { `description` (plain-language trigger), `evaluation_mode`: \"current_state\" | \"change\", `type`: \"llm\" | \"not_empty\" | \"text_contains\" (+`text`) | \"regex\" (+`pattern`) | \"json_predicate\" | \"json_logic\" }.",
        "required": "A monitored target - semantic `description` of what to observe (preferred; AgentArk polls it model-routed), or explicit `poll_action`+`poll_arguments`, or `script` - plus a structured `condition` object: { `description` (plain-language trigger), `evaluation_mode`: \"current_state\" | \"change\", `type`: \"llm\" | \"not_empty\" | \"text_contains\" (+`text`) | \"regex\" (+`pattern`) | \"json_predicate\" | \"json_logic\" }.",
        "optional": [
            "on_trigger", "interval_secs", "notify_channel", "repeat_on_match",
            "script_language", "context_from", "workdir"
        ],
        "example": {
            "description": "Watch the named public sources for material state changes",
            "condition": {
                "type": "llm",
                "evaluation_mode": "change",
                "description": "The observed state changed since the previous successful check"
            },
            "on_trigger": "Notify through the configured channel with the old and new observed values",
            "interval_secs": 43200,
...
fn durable_validation_failure_result(
    tool: &str,
    durable_object: &str,
    detail: &str,
    reason: &str,
...
fn schedule_task_validation_failure_result(detail: &str, reason: &str) -> String {
    schedule_task_validation_failure_with_violations(detail, reason, &[])
}

fn schedule_task_validation_failure_with_violations(
    detail: &str,
    reason: &str,
    violations: &[String],
) -> String {
    durable_validation_failure_result(
        "schedule_task",
        "Scheduled task",
        detail,
        reason,
        violations,
        schedule_task_expected_contract(),
        "Every unsatisfied requirement is listed in data.violations and the full argument contract is in data.expected_contract. Repair ALL of them in one corrected schedule_task call using facts from the current conversation; do not fix one field at a time. For existing task updates, keep the same task_id and repair the schedule fields; do not cancel, delete, or recreate the task as an argument-repair path unless the user explicitly requested cancellation or deletion. Ask the user only when a required fact is genuinely absent from the conversation.",
    )
}

fn watcher_validation_failure_result(detail: &str, reason: &str) -> String {
    durable_validation_failure_result(
        "watch",
        "Watcher",
        detail,
        reason,
        &[],
        watch_expected_contract(),
        "Every unsatisfied requirement is listed in data.violations and the full argument contract is in data.expected_contract. Repair ALL of them in one corrected watch call using facts from the current conversation; do not fix one field at a time. Use the semantic watcher poll contract when the user described what to monitor but did not provide a low-level poll_action/script. Ask the user only when the semantic target or trigger condition is genuinely absent from the conversation.",
    )
}
...
fn watcher_ids_from_watch_completion_data(data: &serde_json::Value) -> Vec<String> {
    data.get("watcher_id")
        .and_then(|value| value.as_str())
        .map(str::trim)
        .filter(|value| !value.is_empty())
...
fn watch_action_execution_result_from_output(
    output: &str,
) -> std::result::Result<serde_json::Value, String> {
    let message = strip_tool_completion_marker_line(output);
    let completion = crate::runtime::parse_watch_completion(output).ok_or_else(|| {
        if message.is_empty() {
            "Watcher creation did not return a structured watch result.".to_string()
        } else {
            message.clone()
        }
    })?;
    let completion_detail = completion
        .detail
        .as_deref()
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .unwrap_or("");
    let readable_message = if message.is_empty() {
        completion_detail.to_string()
    } else {
        message.clone()
    };
    if completion.status.trim() != "completed" {
        return Err(if readable_message.is_empty() {
            "Watcher creation did not complete.".to_string()
        } else {
            readable_message
        });
    }
    let data = schedule_task_completion_data(output)
        .ok_or_else(|| "Watcher creation did not return completion data.".to_string())?;
    let watcher_ids = watcher_ids_from_watch_completion_data(&data);
    if watcher_ids.is_empty() {
        return Err(if readable_message.is_empty() {
            "Watcher creation did not return a durable watcher id.".to_string()
        } else {
            format!(
                "Watcher creation did not return a durable watcher id. {}",
                readable_message
            )
        });
    }
    let watcher_id = watcher_ids.first().cloned();
    Ok(serde_json::json!({
        "status": "executed",
        "kind": "watch",
...
impl Agent {
    ...
    pub(super) fn watcher_runtime_signature(
    ...
    pub async fn apply_autopilot_mode(
        ...
        let mut seen_watcher_signatures: HashSet<String> = existing_watchers
            .iter()
            .filter(|watcher| {
                watcher.status == crate::core::automation::watcher::WatcherStatus::Active
            })
    ...
    pub async fn execute_autonomy_action_payload(
        ...
        match action_kind {
            ...
            "watch" => {
                ...
                watch_action_execution_result_from_output(&output)
            }
            "activate_mode" => {
                let mode_id = payload
                    .get("mode_id")
    ...
    pub(crate) async fn handle_watcher_trigger_supervised(
        ...
        let external_notify_text =
            direct_notification_external_message("watcher", None, &notify_text);
            ...
            || is_external_notification_channel(&authorization_delivery_channel)
        {
            ...
            let notify_arguments = serde_json::json!({
                "message": external_notify_text.clone(),
            ...
            || requested_channel == AUTOMATION_IN_APP_NOTIFICATION_CHANNEL
        {
            ...
            if is_external_notification_channel(&requested_channel) {
                let outcome = if self
                    ...
                    .await
                {
                    self.try_send_notification_reported(&requested_channel, &external_notify_text)
                        .await
                } else {
            ...
            let outcome = self
                .try_send_notification_reported(&requested_channel, &external_notify_text)
                .await;
            if outcome.success {
                if let Some(image) = notification_image.as_ref() {
                    let image_outcome = self
...
Return: 1 short status paragraph + 3 bullet next steps.",
    ...
    pub(crate) fn extract_structured_watcher_condition_payload(
        result: &str,
    ) -> Option<serde_json::Value> {
    ...
    pub(super) fn evaluate_watch_json_predicate(
        payload: &serde_json::Value,
        predicate: &crate::core::automation::watcher::WatchJsonPredicate,
    ) -> Result<bool, String> {
        ...
        match predicate.operator {
            crate::core::automation::watcher::WatchConditionOperator::Eq => {
            ...
            crate::core::automation::watcher::WatchConditionOperator::Ne => {
                Ok(!Self::json_values_equal_relaxed(
                    target,
                    predicate.value.as_ref().ok_or_else(|| {
                        format!("condition path `{}` is missing comparison value", path)
            ...
            crate::core::automation::watcher::WatchConditionOperator::Gt
            | crate::core::automation::watcher::WatchConditionOperator::Gte
            | crate::core::automation::watcher::WatchConditionOperator::Lt
            | crate::core::automation::watcher::WatchConditionOperator::Lte => {
            ...
            crate::core::automation::watcher::WatchConditionOperator::Contains => {
            ...
            crate::core::automation::watcher::WatchConditionOperator::NotContains => {
                Ok(!Self::json_value_contains_expected(
                    target,
                    predicate.value.as_ref().ok_or_else(|| {
                        format!("condition path `{}` is missing contains value", path)
            ...
            crate::core::automation::watcher::WatchConditionOperator::NonEmpty => {
                Ok(Self::json_value_is_non_empty(target))
            }
            crate::core::automation::watcher::WatchConditionOperator::Empty => {
                Ok(!Self::json_value_is_non_empty(target))
            }
            crate::core::automation::watcher::WatchConditionOperator::True => {
            ...
            crate::core::automation::watcher::WatchConditionOperator::False => {
                Ok(!Self::json_value_as_bool(target).ok_or_else(|| {
                    format!(
                        "condition path `{}` is not boolean-like in the poll output",
                        path
            ...
            crate::core::automation::watcher::WatchConditionOperator::Regex => {
            ...
            crate::core::automation::watcher::WatchConditionOperator::Exists
            | crate::core::automation::watcher::WatchConditionOperator::NotExists => unreachable!(),
        }
    }
    ...
    pub(crate) fn evaluate_watch_condition_without_llm(
        ...
        let current_state_match = match &condition.matcher {
            crate::core::automation::watcher::WatchConditionMatcher::NotEmpty => Some(Ok(!trimmed
                .is_empty()
            ...
            crate::core::automation::watcher::WatchConditionMatcher::TextContains {
                text,
                case_sensitive,
            ...
            crate::core::automation::watcher::WatchConditionMatcher::Regex { pattern } => Some(
                Regex::new(pattern)
                    .map(|regex| regex.is_match(trimmed))
                    .map_err(|error| format!("condition regex is invalid: {}", error)),
            ),
            crate::core::automation::watcher::WatchConditionMatcher::JsonPredicate {
                path,
                operator,
                value,
            } => {
                ...
                Some(payload.and_then(|payload| {
                    Self::evaluate_watch_json_predicate(
                        &payload,
                        &crate::core::automation::watcher::WatchJsonPredicate {
                            path: path.clone(),
            ...
            crate::core::automation::watcher::WatchConditionMatcher::JsonLogic { logic, rules } => {
                let payload = Self::extract_structured_watcher_condition_payload(result)
                    .ok_or_else(|| {
                        "poll result did not contain structured JSON output".to_string()
                    });
                Some(payload.and_then(|payload| match logic {
                    crate::core::automation::watcher::WatchConditionLogic::All => {
                        for rule in rules {
                    ...
                    crate::core::automation::watcher::WatchConditionLogic::Any => {
            ...
            crate::core::automation::watcher::WatchConditionMatcher::Llm => None,
        }?;
    ...
    pub(super) fn watch_condition_requires_structured_payload(
        ...
        matches!(
            condition.matcher,
            crate::core::automation::watcher::WatchConditionMatcher::JsonPredicate { .. }
                | crate::core::automation::watcher::WatchConditionMatcher::JsonLogic { .. }
        )
    }
            ...
            let matching_ids = queue
                ...
                .filter(|existing| {
                    !matches!(existing.status, super::task::TaskStatus::Completed)
                        && crate::core::automation::task::tasks_are_semantically_similar(
                            existing, &task,
                        )
                })
    ...
    pub(super) async fn handle_schedule_task(
        ...
        if let Some(batch_items) = schedule_task_batch_item_arguments(arguments) {
            let batch_items = match batch_items {
                Ok(items) => items,
                Err(error) => {
                    // Structured failure (not a bare string): carries the full
                    // contract so one repair turn can fix the whole batch.
                    let detail = error.to_string();
                    let violations = schedule_task_batch_violation_messages(&detail);
                    return Some(schedule_task_validation_failure_with_violations(
                        &detail,
                        "invalid_items",
                        &violations,
                    ));
        ...
        // Whole-payload validation: evaluate every argument requirement before
        // failing, then report ALL violations in one structured result so the
        // model can repair the complete contract in a single corrected call
        // instead of discovering one field per repair turn.
        let task_desc =
        ...
        let default_timezone = schedule_task_default_timezone_from_profile_or_request(
            profile_timezone.as_deref(),
            authorization,
        );
        ...
        let schedule = schedule_task_schedule_from_arguments(arguments, schedule_context);

        let mut violations: Vec<String> = Vec::new();
        if task_desc.is_none() {
            violations.push(
                "`task` (what to do) is required unless `task_id` points at an existing task."
                    .to_string(),
            );
        }
        let (cron_expr, scheduled_for) = match schedule {
        ...
        if let Some(cron) = cron_expr.as_deref() {
            ...
            if requests_subminute {
                violations.push(
                    "Sub-minute recurring schedules are not supported by schedule_task; the minimum cadence is one minute. Use a managed service or background command loop when the workflow genuinely needs sub-minute polling."
                        .to_string(),
                );
            }
        }
        if !violations.is_empty() {
            let reason = if task_desc.is_none() && violations.len() == 1 {
                "missing_task"
            } else if task_desc.is_some() {
                "invalid_schedule"
            } else {
                "invalid_arguments"
            };
        ...
        let mut explicit_action = arguments
            .get("action")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string())
            .or_else(|| {
        ...
        let mut explicit_valid = explicit_action
            .as_ref()
            .map(|name| all_actions.iter().any(|a| a.name == *name))
            .unwrap_or(false);

        if explicit_action.is_some() && !explicit_valid {
            if let Some(normalized_action) = normalize_invalid_scheduled_notification_action(
                ...
                explicit_action = Some(normalized_action);
                explicit_valid = true;
            } else {
                return Some(format!(
    ...
    pub async fn handle_watch(
        ...
        if let Some(batch_items) = watch_batch_item_arguments(arguments) {
            let batch_items = match batch_items {
                Ok(items) => items,
                Err(error) => {
                    return Some(watcher_validation_failure_result(
                        &error.to_string(),
                        "invalid_batch",
                    ));
                }
        ...
        let condition = if let Some(value) = arguments.get("condition") {
            match serde_json::from_value::<crate::core::automation::watcher::WatchCondition>(
                value.clone(),
            ) {
        ...
        if let Err(error) = condition.validate() {
            return Some(watcher_validation_failure_result(
                &format!("Watcher condition is invalid: {}", error),
                "invalid_condition",
            ));
        }
        ...
        let semantic_poll_contract = if existing_watcher_target.is_none()
            && script_poll_arguments.is_none()
            && arguments
                .get("poll_action")
                .and_then(|v| v.as_str())
                .map(str::trim)
                ...
                .is_none()
        {
            semantic_watcher_poll_contract_from_arguments(
                arguments,
                &description,
                &condition,
                &on_trigger,
        ...
        let Some(poll_action_value) = arguments
            ...
            .or_else(|| {
                existing_watcher_target
                    ...
                    .or_else(|| {
                        semantic_poll_contract
                            .as_ref()
                            .map(|contract| contract.poll_action.clone())
                    })
            })
        else {
            return Some(watcher_validation_failure_result(
                "Watcher requires a monitored target. Provide a semantic description of what to observe, or supply a custom polling implementation.",
                "missing_poll_target",
            ));
        };
        ...
        let mut poll_arguments = arguments
            ...
            .or_else(|| {
                semantic_poll_contract
                    .as_ref()
                    .map(|contract| contract.poll_arguments.clone())
            })
            .unwrap_or_else(|| serde_json::json!({}));
        ...
        if !watcher_action_is_semantic_poll(&poll_action) {
            ...
            .await
        {
            return Some(watcher_validation_failure_result(
                &error,
                "background_session_policy_rejected",
            ));
        }
        ...
        if !watcher_action_is_semantic_poll(&poll_action) {
            if let Err(error) = self
                .runtime
                .validate_action_invocation_with_context(
                    &poll_action,
                    &poll_arguments,
                    &watcher_auth,
                )
                .await
            {
                return Some(watcher_validation_failure_result(
                    ...
                    "poll_action_not_runnable",
                ));
            }
        }
        let mut initial_baseline_result: Option<String> = None;
        if !watcher_action_is_semantic_poll(&poll_action)
            && (poll_action.eq_ignore_ascii_case("code_execute")
                ...
                || Self::watch_condition_requires_previous_result(&condition))
        {
            ...
            if !critique.validation_passed {
                return Some(watcher_validation_failure_result(
                    ...
                    "poll_preflight_validation_failed",
                ));
            }
                ...
                Self::evaluate_watch_condition_without_llm(&condition, &preflight_output, None)
            {
                return Some(watcher_validation_failure_result(
                    ...
                    "condition_poller_contract_mismatch",
                ));
            }
...
mod tests {
    ...
    fn test_task(
        action: &str,
        arguments: serde_json::Value,
    ) -> crate::core::automation::task::Task {
        crate::core::automation::task::Task {
            id: uuid::Uuid::new_v4(),
            description: "Scheduled test task".to_string(),
            action: action.to_string(),
            arguments,
            approval: crate::core::automation::task::TaskApproval::Auto,
            capabilities: vec![action.to_string()],
            status: crate::core::automation::task::TaskStatus::Pending,
            created_at: chrono::Utc::now(),
    ...
    #[test]
    fn schedule_task_validation_failure_carries_violations_and_full_contract() {
        let violations = vec![
            "`task` (what to do) is required unless `task_id` points at an existing task."
                .to_string(),
            "schedule_task requires `cron`, `at`, `scheduled_for`, or `local_time`".to_string(),
        ];
        let result = schedule_task_validation_failure_with_violations(
            "Task scheduling needs a task and a schedule.",
            "invalid_arguments",
            &violations,
        );
        let payload = result
        ...
        let listed = payload["data"]["violations"]
            .as_array()
            .expect("violations array");
        assert_eq!(listed.len(), 2);
        assert!(payload["data"]["expected_contract"]["required"]
            .as_str()
            .unwrap_or_default()
            .contains("cron"));
        assert!(payload["data"]["assistant_instruction"]
            .as_str()
            .unwrap_or_default()
            .contains("expected_contract"));
        assert_eq!(payload["data"]["recoverable_by_model"], true);
    }

    #[test]
    fn watcher_validation_failure_carries_full_contract() {
        let result = watcher_validation_failure_result(
            "Watcher requires a monitored target.",
            "missing_poll_target",
        );
        let payload = result
            .trim_start()
        ...
        assert!(payload["data"]["expected_contract"]["required"]
            ...
            .unwrap_or_default()
            .contains("condition"));
        assert!(payload["data"]["expected_contract"]["shape"]
            .as_str()
            .unwrap_or_default()
    ...
    #[test]
    fn schedule_task_contract_examples_pass_their_own_validators() {
        // Parity pin: the examples we TEACH the model must provably satisfy the
        // validators. If a validator requirement changes without updating the
        // contract, this test fails.
        let contract = schedule_task_expected_contract();

        let example = contract.get("example").expect("contract example");
        assert!(schedule_task_has_task_source(example));
        assert!(schedule_task_has_schedule_source(example));

        let batch_example = contract.get("batch_example").expect("batch example");
        let items = schedule_task_batch_item_arguments(batch_example)
            .expect("batch example uses items")
            .expect("batch example must pass item validation");
        assert_eq!(items.len(), 2);
        for item in &items {
            assert!(schedule_task_has_task_source(item));
            assert!(schedule_task_has_schedule_source(item));
        }
    }

    #[test]
    fn schedule_task_contract_example_teaches_executable_shape() {
        let contract = schedule_task_expected_contract();
        let example = contract.get("example").expect("contract example");

        assert_eq!(
            example.get("action").and_then(|value| value.as_str()),
            Some("notify_user"),
            "schedule_task examples must show the executable action shape instead of relying on scheduler defaults"
        );
        assert!(example
            .pointer("/action_arguments/message")
            .and_then(|value| value.as_str())
            .map(str::trim)
            .is_some_and(|value| !value.is_empty()));
        assert!(
            example.get("script").is_none(),
            "the default example should stay a pure scheduled notification; monitoring examples belong in watch"
        );
    }

    #[test]
    fn watch_contract_example_matches_watcher_validator_and_delivery_field() {
        let contract = watch_expected_contract();
        ...
        let optional = contract["optional"]
            .as_array()
            .expect("watch optional fields");
        assert!(optional.iter().any(|value| value == "notify_channel"));
        assert!(!optional.iter().any(|value| value == "report_to"));

        let example = contract.get("example").expect("watch example");
        assert_eq!(
            example
                .get("notify_channel")
                .and_then(|value| value.as_str()),
            Some("in_app"),
            "watch uses notify_channel for delivery"
        );
        assert_eq!(
            example
                .pointer("/condition/evaluation_mode")
                .and_then(|value| value.as_str()),
            Some("change"),
            "change-only monitoring should teach the condition evaluation mode explicitly"
        );
        let condition: crate::core::automation::watcher::WatchCondition =
            serde_json::from_value(example["condition"].clone()).expect("watch condition example");
        condition
            .validate()
            .expect("watch condition example must satisfy validator");
        let contract = semantic_watcher_poll_contract_from_arguments(
            example,
            example["description"].as_str().expect("description"),
            &condition,
            example["on_trigger"].as_str().expect("on_trigger"),
        )
        .expect("watch example should build semantic poll contract");
        assert_eq!(contract.poll_action, SEMANTIC_WATCHER_POLL_ACTION);
    }

    #[test]
    fn schedule_task_batch_items_report_all_violations_at_once() {
        let arguments = serde_json::json!({
            "items": [
                {},
                { "task": "Morning pricing check", "cron": "0 10 * * *" },
                { "cron": "0 22 * * *" }
            ]
        });

        let error = schedule_task_batch_item_arguments(&arguments)
            .expect("items present")
            .expect_err("invalid items must fail validation");
        let message = error.to_string();

        assert!(message.contains("items[0]"));
        assert!(message.contains("items[2]"));
        assert!(!message.contains("items[1]"));

        let violations = schedule_task_batch_violation_messages(&message);
        let result = schedule_task_validation_failure_with_violations(
            &message,
            "invalid_items",
            &violations,
        ...
        let payload = result
            ...
            .strip_prefix(crate::runtime::TOOL_COMPLETION_MARKER)
            .and_then(|payload| serde_json::from_str::<serde_json::Value>(payload).ok())
            .expect("structured validation failure");
        assert_eq!(
    ...
    #[test]
    fn invalid_explicit_delivery_action_can_normalize_to_notify_user() {
        let all_actions = vec![action("notify_user", "Notify the user.", &[])];
        let mut task_args = serde_json::json!({
            "query": "Reminder: meeting with Mark",
            "report_to": "ext.ops-alerts"
        });

        let selected = normalize_invalid_scheduled_notification_action(
            Some("send_external_delivery_channel"),
            &mut task_args,
            "Reminder: meeting with Mark",
            "ext.ops-alerts",
            &all_actions,
        );

        assert_eq!(selected.as_deref(), Some("notify_user"));
        assert_eq!(
            task_args.get("message").and_then(|value| value.as_str()),
            Some("Reminder: meeting with Mark")
        );
        assert_eq!(
            task_args.get("report_to").and_then(|value| value.as_str()),
            Some("ext.ops-alerts")
        );
    }
    ...
    #[test]
    fn schedule_local_time_accepts_common_clock_notation_variants() {
        let cases = [
            ("21:00 hrs", "21:00:00"),
            ("21\"00 hrs", "21:00:00"),
            ("21hrs", "21:00:00"),
            ("21h30", "21:30:00"),
            ("9pm", "21:00:00"),
            ("9 night time", "21:00:00"),
        ];

        for (raw, expected) in cases {
            let parsed = parse_schedule_local_time(raw)
                .unwrap_or_else(|error| panic!("expected `{}` to parse: {}", raw, error));
            let expected =
                chrono::NaiveTime::parse_from_str(expected, "%H:%M:%S").expect("test time");
            assert_eq!(parsed, expected, "input `{}`", raw);
        }
    }

    #[test]
    fn schedule_task_default_timezone_uses_request_context_when_profile_unset() {
        let request_context = crate::actions::ActionAuthorizationContext {
            request_timezone: Some("Asia/Kolkata".to_string()),
            request_timezone_offset_minutes: Some(330),
            ..crate::actions::ActionAuthorizationContext::default()
        };

        let timezone =
            schedule_task_default_timezone_from_profile_or_request(None, Some(&request_context))
                .expect("request timezone should be accepted");
        let now_utc = chrono_tz::Asia::Kolkata
            .with_ymd_and_hms(2026, 6, 3, 21, 52, 0)
            .single()
            .unwrap()
            .with_timezone(&chrono::Utc);
        let context = ScheduleTaskScheduleContext {
        ...
        let (cron, at) = schedule_task_schedule_from_arguments(
            &serde_json::json!({
                "task": "Send reminder",
                "local_time": "10 PM",
                "date_policy": "next_occurrence"
            }),
            context,
        )
        .expect("time-only create should use request timezone");

        assert!(cron.is_none());
        assert_eq!(at.unwrap().to_rfc3339(), "2026-06-03T16:30:00+00:00");
    }

    #[test]
    fn schedule_task_default_timezone_prefers_saved_profile() {
        let request_context = crate::actions::ActionAuthorizationContext {
            request_timezone: Some("Asia/Kolkata".to_string()),
            request_timezone_offset_minutes: Some(330),
            ..crate::actions::ActionAuthorizationContext::default()
        ...
        let timezone = schedule_task_default_timezone_from_profile_or_request(
            Some("UTC"),
            Some(&request_context),
        )
        .expect("saved profile timezone should be accepted");
        let now_utc = chrono_tz::Asia::Kolkata
            .with_ymd_and_hms(2026, 6, 3, 21, 52, 0)
            .single()
            .unwrap()
        ...
        .expect("time-only create should prefer saved profile timezone");

        assert!(cron.is_none());
        assert_eq!(at.unwrap().to_rfc3339(), "2026-06-03T22:00:00+00:00");
    }
    ...
    #[test]
    fn watcher_batch_accepts_semantic_items_without_low_level_poll_action() {
        let args = serde_json::json!({
            "condition": {
                "description": "Observed state changes in a way the user cares about",
                "evaluation_mode": "change",
                "type": "llm"
            },
            "on_trigger": "Notify with the detected change.",
            "interval_secs": 3600,
            "notify_channel": "in_app",
            "items": [
                {
                    "description": "Monitor a public status page for relevant changes.",
                    "source": {"url": "https://example.com/status"}
                },
                {
                    "description": "Monitor a project feed for relevant changes.",
                    "source": {"url": "https://example.org/feed"}
                }
            ]
        });
        ...
        let items = watch_batch_item_arguments(&args)
            .expect("batch should be detected")
            .expect("semantic batch items should validate");

        assert_eq!(items.len(), 2);
        assert_eq!(
            items[0].get("poll_action").and_then(|value| value.as_str()),
            None
        );
        assert_eq!(
            items[0]["condition"]["evaluation_mode"].as_str(),
            Some("change")
        );
        assert_eq!(
            items[1]["source"]["url"].as_str(),
            Some("https://example.org/feed")
        );
    }

    #[test]
    fn watcher_semantic_contract_supplies_poll_when_action_is_omitted() {
        let condition = crate::core::automation::watcher::WatchCondition {
            description: "Trigger when the observed state materially changes".to_string(),
            evaluation_mode: crate::core::automation::watcher::WatchConditionEvaluationMode::Change,
            matcher: crate::core::automation::watcher::WatchConditionMatcher::Llm,
        };
        let args = serde_json::json!({
            "description": "Monitor public product and pricing pages for plan or tier changes.",
            "condition": condition,
            "on_trigger": "Notify the user with the detected differences.",
            "interval_secs": 43200,
            "notify_channel": "in_app",
            "metadata": {
                "targets": [
                    {"label": "Provider A", "url": "https://example.com/pricing"},
                    {"label": "Provider B", "url": "https://example.org/pricing"}
                ]
            }
        });
        ...
        let contract = semantic_watcher_poll_contract_from_arguments(
            &args,
            "Monitor public product and pricing pages for plan or tier changes.",
            &condition,
            "Notify the user with the detected differences.",
        )
        .expect("semantic watcher intent should produce an internal poll contract");

        assert_eq!(contract.poll_action, SEMANTIC_WATCHER_POLL_ACTION);
        assert_eq!(
            contract.poll_arguments["semantic_watcher_poll"],
            serde_json::Value::Bool(true)
        );
        assert_eq!(
            contract.poll_arguments["intent"]["description"].as_str(),
            Some("Monitor public product and pricing pages for plan or tier changes.")
        );
        assert_eq!(
            contract.poll_arguments["intent"]["condition"]["evaluation_mode"].as_str(),
            Some("change")
        );
        assert_eq!(
            contract.poll_arguments["source_payload"]["metadata"]["targets"][0]["url"].as_str(),
            Some("https://example.com/pricing")
        );
    }

    #[test]
    fn watcher_validation_failures_are_structured_failed_completions() {
        let result = watcher_validation_failure_result(
            "Watcher setup payload was incomplete.",
            "missing_poll_target",
        );

        let completion = crate::runtime::parse_watch_completion(&result)
            .expect("validation failure should use a structured watch marker");
        assert_eq!(completion.tool, "watch");
        assert_eq!(completion.status, "failed");
        assert!(result.contains("durable_commit"));
        assert!(result.contains("recoverable_by_model"));
        assert!(result.contains("missing_poll_target"));
    }

    #[test]
    fn watch_action_execution_result_rejects_failed_completion() {
        let output = watcher_validation_failure_result(
            "Watcher setup payload was incomplete.",
            "missing_poll_target",
        );

        let error = watch_action_execution_result_from_output(&output)
            .expect_err("failed watch completion must not be treated as executed");

        assert!(error.contains("Watcher setup payload was incomplete."));
    }

    #[test]
    fn watch_action_execution_result_requires_durable_watcher_id() {
        let output = format!(
            "{}{}\nCreated the background watcher.",
        ...
        let error = watch_action_execution_result_from_output(&output)
            .expect_err("completed watch output without watcher id must fail");

        assert!(error.contains("durable watcher id"));
    }

    #[test]
    fn watch_action_execution_result_accepts_batch_watcher_ids() {
        let output = format!(
            "{}{}\nSaved 2 watcher(s).",
            crate::runtime::TOOL_COMPLETION_MARKER,
            serde_json::json!({
        ...
        let result = watch_action_execution_result_from_output(&output)
            .expect("batch watch output with ids should be accepted");

        assert_eq!(result["watcher_id"].as_str(), Some("watch-a"));
        assert_eq!(
            result["watcher_ids"],
            serde_json::json!(["watch-a", "watch-b"])
        );
    }
    ...
    fn watcher_change_mode_requires_baseline_and_detects_difference() {
        let condition = crate::core::automation::watcher::WatchCondition {
            ...
            evaluation_mode: crate::core::automation::watcher::WatchConditionEvaluationMode::Change,
            matcher: crate::core::automation::watcher::WatchConditionMatcher::NotEmpty,
        };
