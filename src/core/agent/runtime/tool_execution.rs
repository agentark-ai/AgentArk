...
fn background_session_blocks_gepa_idle(
    session: &crate::core::automation::background_session::BackgroundSession,
    now: chrono::DateTime<chrono::Utc>,
    quiet_window_seconds: i64,
) -> bool {
    if !matches!(
        session.status,
        crate::core::automation::background_session::BackgroundSessionStatus::Active
    ) {
        return false;
    }
...
fn list_watchers_status_label(
    status: &crate::core::automation::watcher::WatcherStatus,
) -> &'static str {
    match status {
...
fn list_watchers_live_row(
    watcher: &crate::core::automation::watcher::Watcher,
) -> serde_json::Value {
    let status_error = match &watcher.status {
        crate::core::automation::watcher::WatcherStatus::Failed { error } => Some(error.clone()),
        _ => None,
    };
    serde_json::json!({
...
pub(crate) fn tool_start_context_id_key(
    call: &crate::core::model::llm::ToolCall,
) -> Option<String> {
    let id = call.id.trim();
    if id.is_empty() {
        None
...
pub(crate) fn tool_start_context_signature_key(call: &crate::core::model::llm::ToolCall) -> String {
...
fn watcher_reference_blob(watcher: &crate::core::automation::watcher::Watcher) -> String {
...
impl Agent {
    ...
    async fn raise_missing_secret_chat_prompt(
        ...
        match crate::core::connectivity::integration_auth::resolve_secret_key_among_manifests(
            &storage_key,
            &manifests,
        ) {
            crate::core::connectivity::integration_auth::ReverseLookupOutcome::Unique(manifest) => {
                let has_form_fields = match &manifest.mode {
                    ...
                    | crate::core::connectivity::integration_auth::AuthMode::OAuth2DeviceCode(_) => false,
                };
                if has_form_fields {
            ...
            crate::core::connectivity::integration_auth::ReverseLookupOutcome::Ambiguous {
                key,
                candidates,
            } => {
                tracing::warn!(
            ...
            crate::core::connectivity::integration_auth::ReverseLookupOutcome::None => {
                self.remember_raw_secret_chat_prompt(
                    conversation_id,
                    &storage_key,
                    Some(tool_name),
    ...
    pub(crate) fn tool_call_signature(call: &crate::core::model::llm::ToolCall) -> String {
        ...
        if normalized_name == "watch" {
            ...
            return format!(
                "watch:{}{}",
                target,
                crate::core::automation::watcher::watcher_tool_call_signature_from_arguments(
                    &call.arguments
                )
            );
        }
        if normalized_name == "schedule_task" {
            ...
            return format!(
                "schedule_task:{}{}",
                target,
                crate::core::automation::task::task_request_signature_from_fields(
                    action_name,
                    description,
                    &action_arguments,
                    cron_expr,
    ...
    pub(crate) async fn record_self_tune_autonomous_success(&self) {
        crate::core::self_evolve::self_tune::record_autonomous_success(&self.storage).await;
    }

    pub(crate) async fn record_self_tune_user_rejection(&self) {
        crate::core::self_evolve::self_tune::record_user_rejection(&self.storage).await;
    }

    fn delegated_result_text_for_tool_output(
        result: &crate::core::automation::task_router::DelegatedResult,
    ) -> String {
    ...
    async fn handle_delegate_tool_call(
        ...
        let decision = crate::core::automation::task_router::RoutingDecision {
        ...
        match self
            ...
            .execute(
                &decision,
                crate::core::automation::task_router::TaskRouterExecuteContext {
            ...
            .await
        {
            Ok(crate::core::automation::task_router::TaskRouterResult::Delegated(result)) => {
                let agents = result
                    .agent_results
                    .iter()
                    .map(|item| {
            ...
            Ok(crate::core::automation::task_router::TaskRouterResult::Direct) => {
                serde_json::json!({
                    "ok": false,
    ...
    async fn load_video_bytes(&self, source_url: &str, max_bytes: usize) -> Result<Vec<u8>> {
        ...
        if let Ok(parsed) = reqwest::Url::parse(source_url) {
            ...
            let client = crate::core::runtime::net::SafeHttpClient::new(90)?;
    ...
    async fn stop_and_remove_existing_app(
        ...
        if let Err(error) = crate::actions::app::remove_app_scoped_env_secrets(
            &self.config_dir,
            &self.data_dir,
            app_id,
        ) {
    ...
    fn app_quality_tool_summary(
        ...
        let concerns = report
            ...
            .unwrap_or_default();
        let needs_repair = status == "error"
            || (request_context_attached && effective_judge_passed == Some(false));
        serde_json::json!({
            "status": status,
            "request_context_attached": request_context_attached,
            "acceptance_criteria_count": acceptance_criteria_count,
    ...
    async fn build_app_coverage_report(
        ...
        let items = goals
            ...
            .map(|(idx, goal)| {
                let score =
                    crate::core::knowledge::document_search::normalized_embedding_similarity(
    ...
    fn capability_acquire_has_structured_contract(arguments: &serde_json::Value) -> bool {
    ...
    fn capability_acquire_text_field(arguments: &serde_json::Value, key: &str) -> Option<String> {
        arguments
            .get(key)
            .and_then(|value| value.as_str())
            .map(str::trim)
    ...
    fn capability_acquire_source_request(
        arguments: &serde_json::Value,
    ) -> Option<crate::custom_apis::CustomApiPreviewRequest> {
        if Self::capability_acquire_has_structured_contract(arguments) {
            return None;
        }

        let name = Self::capability_acquire_text_field(arguments, "name")
            .or_else(|| Self::capability_acquire_text_field(arguments, "id"));
        let base_url = Self::capability_acquire_text_field(arguments, "base_url");
        let source_alias = crate::core::request_contract::source_alias_value(arguments);
        let mut source = None;
        let mut openapi_url = None;
        let mut openapi_text = None;
        let mut curl_text = None;
        if let Some((key, value)) = source_alias {
            match key {
                "openapi_url" => openapi_url = Some(value),
                "openapi_text" => openapi_text = Some(value),
                "curl_text" => curl_text = Some(value),
                _ => source = Some(value),
            }
        }
        ...
        if source.is_none()
            && openapi_url.is_none()
            && openapi_text.is_none()
            && curl_text.is_none()
        {
            return None;
        }

        Some(crate::custom_apis::CustomApiPreviewRequest {
            name,
            base_url,
            source,
            openapi_url,
    ...
    fn custom_api_auth_mode_argument(mode: crate::custom_apis::CustomApiAuthMode) -> String {
    ...
    async fn capability_acquire_arguments_with_source_contract(
        &self,
        arguments: &serde_json::Value,
    ) -> Result<serde_json::Value> {
        let Some(request) = Self::capability_acquire_source_request(arguments) else {
            return Ok(arguments.clone());
        };
        let preview =
            crate::custom_apis::preview_custom_api_with_model(request, Some(&self.llm)).await?;
        ...
        enriched
            ...
            .or_insert_with(|| serde_json::Value::String(preview.suggested_name.clone()));
        enriched
            .entry("id".to_string())
            .or_insert_with(|| serde_json::Value::String(preview.suggested_id));
        enriched.insert(
            "base_url".to_string(),
            serde_json::Value::String(preview.base_url),
        );
        enriched.insert(
            "auth_type".to_string(),
            serde_json::Value::String(Self::custom_api_auth_mode_argument(preview.auth_mode)),
        );
        if let Some(auth_header) = preview.auth_header.or(preview.auth_name) {
            enriched.insert(
                "auth_header_name".to_string(),
                serde_json::Value::String(auth_header),
            );
        }
        if let Some(auth_username) = preview.auth_username {
            enriched.insert(
                "auth_username".to_string(),
                serde_json::Value::String(auth_username),
            );
        }
        enriched.insert(
            "operations".to_string(),
            serde_json::to_value(preview.operations)?,
        );
        enriched.insert(
            "_capability_source_evidence".to_string(),
            serde_json::json!(["source_contract"]),
        );
        Ok(serde_json::Value::Object(enriched))
    }

    async fn handle_capability_acquire_tool_call(
        &self,
        arguments: &serde_json::Value,
        authorization: Option<&crate::actions::ActionAuthorizationContext>,
    ) -> Result<String> {
        let enriched = self
            .capability_acquire_arguments_with_source_contract(arguments)
            .await?;
        let payload = if let Some(auth_context) = authorization {
            self.runtime
                .execute_action_payload_with_context("capability_acquire", &enriched, auth_context)
                .await?
        } else {
            self.runtime
                .execute_action_payload("capability_acquire", &enriched)
                .await?
        };
        Ok(
    ...
    async fn handle_skill_marketplace_manage_action(
        ...
        match operation.as_str() {
            "create" => {
                ...
                let request =
                    crate::core::platform::skill_marketplaces::SkillMarketplaceUpsertRequest {
                ...
                let (status, marketplace) =
                    crate::core::platform::skill_marketplaces::upsert_marketplace(
                        &storage, None, request,
                    )
                    .await?;
                Ok(serde_json::json!({
                    "status": status,
            ...
            "update" => {
                ...
                let marketplaces =
                    crate::core::platform::skill_marketplaces::load_marketplaces(&storage).await?;
                ...
                let request =
                    crate::core::platform::skill_marketplaces::SkillMarketplaceUpsertRequest {
                        ...
                        url: action_text_arg(arguments, "url")
            ...
            "delete" => {
                ...
                let existing = resolve_marketplace(&marketplaces, &target)?;
                let existing_id = existing.id.clone();
                crate::core::platform::skill_marketplaces::remove_marketplace(
                    &storage,
                    &existing_id,
                )
                .await?;
            ...
            "refresh" => {
                ...
                let existing = resolve_marketplace(&marketplaces, &target)?;
                let existing_id = existing.id.clone();
                let marketplace = crate::core::platform::skill_marketplaces::refresh_marketplace(
                    &storage,
                    &existing_id,
                )
                .await?;
    ...
    async fn wait_for_browser_auto_checkpoint(
        &self,
        session_id: &str,
    ) -> Option<crate::core::connectivity::browser_session::BrowserSessionView> {
    ...
    fn browser_auto_notification_fn(
        ...
        dyn Fn(crate::core::connectivity::browser_session::BrowserSessionNotification)
            + Send
            + Sync,
    > {
        ...
        std::sync::Arc::new(move |notification| {
            ...
            let kind = match notification.kind {
                crate::core::connectivity::browser_session::BrowserSessionNotificationKind::Progress => {
                    "progress"
                }
                crate::core::connectivity::browser_session::BrowserSessionNotificationKind::NeedsInput => {
                    "needs_input"
                }
                crate::core::connectivity::browser_session::BrowserSessionNotificationKind::Notice => "notice",
                crate::core::connectivity::browser_session::BrowserSessionNotificationKind::Completed => {
                    "completed"
                }
                crate::core::connectivity::browser_session::BrowserSessionNotificationKind::Failed => "failed",
                crate::core::connectivity::browser_session::BrowserSessionNotificationKind::Closed => "closed",
            };
            queue_stream_event(
                tx,
                StreamEvent::ToolProgress {
    ...
    async fn handle_browser_auto_tool_call(
        ...
        let profile_context = profile
            .as_ref()
            .map(crate::core::connectivity::browser_session::BrowserSessionProfileContext::from_browser_profile);
        ...
        if let Some(profile) = profile.as_ref() {
            let _ = crate::core::BrowserProfileControlPlane::record_session(
                ...
                crate::core::BrowserProfileSessionRecord {
                    ...
                    url: explicit_url.clone().or_else(|| {
                        recent_artifact
                            .as_ref()
    ...
    pub(crate) async fn execute_action_with_hooks(
            ...
            || action_name.eq_ignore_ascii_case("background_work_manage")
        {
            ...
            self.handle_manage_actions_tool_call(arguments, authorization)
                .await
        } else if action_name.eq_ignore_ascii_case("capability_acquire") {
            self.handle_capability_acquire_tool_call(arguments, authorization)
                .await
    ...
    async fn load_public_base_url(&self) -> Option<String> {
        ...
        if self.config.deployment_mode
            == crate::core::runtime::config::DeploymentMode::InternetFacing
        {
    ...
    fn build_internal_control_client() -> Result<reqwest::Client> {
        crate::core::runtime::net::build_internal_control_client(5)
    }
    ...
    fn trigger_arkpulse_refresh(&self, reason: &'static str) {
        ...
        crate::spawn_logged!("src/core/agent/tool_execution.rs:3560", async move {
            let client = match crate::core::runtime::net::build_internal_control_client(4) {
                Ok(c) => c,
                Err(e) => {
    ...
    async fn execute_gepa_import(
        ...
        if !imported.prompt_fragment_candidates.is_empty() {
            let current_fragment_raw = self
                .storage
                .get(crate::core::model::prompt_fragments::PROMPT_FRAGMENT_BUNDLE_PROFILE_KEY)
                .await
                .ok()
                .flatten();
            let result =
                ...
                if let Some(bundle) = result.promoted_prompt_fragment_bundle.as_ref() {
                    ...
                    if let Some(existing_baseline) = current_fragment_raw.as_ref() {
                        let _ = self
                            .storage
                            .set(
                                crate::core::model::prompt_fragments::PROMPT_FRAGMENT_BUNDLE_BASELINE_SNAPSHOT_KEY,
                                existing_baseline,
                            )
                            .await;
                    }
                    let baseline_bundle_version = current_fragment_raw
                        .as_ref()
                        .and_then(|raw| {
                            crate::core::model::prompt_fragments::parse_prompt_fragment_bundle_profile(raw)
                                .map(|bundle| bundle.version)
                        })
                        .unwrap_or_else(|| result.baseline_version.clone());
                    let baseline_version =
                    ...
                    self.storage
                        .set(
                            crate::core::model::prompt_fragments::PROMPT_FRAGMENT_BUNDLE_PROFILE_CANARY_KEY,
                            &candidate_serialized,
                        )
                        .await?;
                    let state = crate::core::self_evolve::strategy_runtime::CanaryRolloutState {
            ...
            if let Ok(bytes) = serde_json::to_vec(&value) {
                let _ = self
                    .storage
                    .set(
                        crate::core::model::prompt_fragments::PROMPT_FRAGMENT_BUNDLE_LAST_RESULT_KEY,
                        &bytes,
                    )
                    .await;
            }
...
fn browser_auto_task_description(
    ...
    let explicit_url = url.map(str::trim).filter(|value| !value.is_empty());
    let recent_artifact = if explicit_url.is_none() {
...
fn browser_auto_checkpoint_kind(
    view: &crate::core::connectivity::browser_session::BrowserSessionView,
...
fn resolve_marketplace<'a>(
    marketplaces: &'a [crate::core::platform::skill_marketplaces::SkillMarketplace],
    target: &str,
) -> Result<&'a crate::core::platform::skill_marketplaces::SkillMarketplace> {
...
fn durable_orchestration_action_result(
    ...
    match handler_result {
        Some(result)
            if is_structured_completion(result.trim_start())
                && durable_structured_completion_is_terminal_safe(result.trim_start()) =>
        {
            result
        }
        Some(result) => {
            let detail = result.trim();
            // Backstop arm for non-structured handler strings: teach the full
            // argument contract here too, so even legacy bare-string failures
            // are repairable in one turn.
            let expected_contract = match action_name {
                "schedule_task" => Some(super::task_runtime::schedule_task_expected_contract()),
                "watch" => Some(super::task_runtime::watch_expected_contract()),
                _ => None,
            };
            render_tool_completion_marker_with_data(
                action_name,
                ...
                serde_json::json!({
                    ...
                    "assistant_instruction": "Treat this as repair evidence. Repair the durable action arguments from the current conversation context when possible; when data.expected_contract is present, satisfy the complete contract in one corrected call instead of fixing fields one at a time. Ask the user in your own words only if a required fact is genuinely absent."
                }),
            )
        }
...
fn durable_structured_completion_is_terminal_safe(result: &str) -> bool {
    let Some(outcome) = super::tool_responses::structured_tool_result_outcome(result) else {
        return true;
    };
    if outcome.state != super::tool_responses::StructuredToolOutcomeState::NeedsInput {
        return true;
    }
    super::tool_responses::extract_json_object_from_text(result)
        .as_ref()
        .and_then(super::tool_responses::structured_user_question_from_value)
        .is_some()
}
...
mod tests {
    ...
    fn background_session_with_activity(
        status: crate::core::automation::background_session::BackgroundSessionStatus,
        last_activity_at: chrono::DateTime<chrono::Utc>,
    ) -> crate::core::automation::background_session::BackgroundSession {
        crate::core::automation::background_session::BackgroundSession {
            ...
            policy: crate::core::automation::background_session::BackgroundSessionPolicy::default(),
    ...
    fn stale_active_background_sessions_do_not_block_gepa_idle() {
        ...
        assert!(!background_session_blocks_gepa_idle(
            &background_session_with_activity(
                crate::core::automation::background_session::BackgroundSessionStatus::Waiting,
                now,
            ),
            now,
            quiet_window_seconds,
    ...
    fn browser_auto_checkpoint_kind_uses_structured_session_fields() {
        fn view(
            ...
            reason: Option<&str>,
            summary: Option<&str>,
        ) -> crate::core::connectivity::browser_session::BrowserSessionView {
            crate::core::connectivity::browser_session::BrowserSessionView {
    ...
    fn durable_orchestration_plain_handler_message_is_not_success() {
        ...
        let payload = result
            ...
            .expect("durable wrapper should return structured payload");
        assert_eq!(
            payload["data"]["durable_commit"],
            serde_json::Value::Bool(false)
        );
        assert_eq!(
            payload["data"]["recoverable_by_model"],
    ...
    #[test]
    fn durable_orchestration_structured_question_can_pause_for_user() {
        let question = render_tool_completion_marker_with_data(
            "watch",
            "needs_input",
            "Choose the account to monitor.",
            json!({
                "session": {
                    "question": "Which account should I monitor?"
                }
            }),
        );
        let result =
            durable_orchestration_action_result("watch", "Watcher", Some(question), |value| {
                crate::runtime::parse_watch_completion(value).is_some()
            });
        let completion =
            crate::runtime::parse_watch_completion(&result).expect("watch marker should parse");
    ...
    #[test]
    fn durable_orchestration_needs_input_without_question_becomes_repairable_failure() {
        let raw_needs_input = render_tool_completion_marker_with_data(
            "watch",
            "needs_input",
    ...
    #[test]
    fn durable_orchestration_structured_failed_marker_passes_through_verbatim() {
        let failed = render_tool_completion_marker_with_data(
            "watch",
            "failed",
        ...
        // A handler-authored structured failure is terminal-safe evidence: the
        // wrapper must not re-wrap or re-classify it.
    ...
    #[test]
    fn capability_acquire_docs_url_without_contract_becomes_generic_source_request() {
        let request = Agent::capability_acquire_source_request(&json!({
            "id": "provider",
            "docs_url": "https://provider.example.dev/api"
        }))
        .expect("source request should be created");

        assert_eq!(request.name.as_deref(), Some("provider"));
        assert_eq!(
            request.source.as_deref(),
            Some("https://provider.example.dev/api")
        );
        assert!(request.openapi_url.is_none());
        assert!(request.curl_text.is_none());
    }

    #[test]
    fn capability_acquire_uses_shared_source_alias_contract() {
        for key in crate::core::request_contract::SOURCE_ALIAS_KEYS {
            let mut object = serde_json::Map::new();
            object.insert("id".to_string(), json!("provider"));
            object.insert(
                (*key).to_string(),
                json!("https://provider.example.dev/contract"),
            );
            let request =
                Agent::capability_acquire_source_request(&serde_json::Value::Object(object))
                    .expect("source request should be created");
            ...
            match *key {
                "openapi_url" => {
                    assert_eq!(
                        ...
                        Some("https://provider.example.dev/contract")
                    );
                    assert!(request.source.is_none());
                }
                "openapi_text" => {
                    assert_eq!(
                        request.openapi_text.as_deref(),
                        Some("https://provider.example.dev/contract")
                    );
                    assert!(request.source.is_none());
                }
    ...
    #[test]
    fn capability_acquire_existing_operation_contract_is_not_reparsed_from_source() {
        let request = Agent::capability_acquire_source_request(&json!({
            "id": "provider",
            "docs_url": "https://provider.example.dev/api",
            "operations": [{
