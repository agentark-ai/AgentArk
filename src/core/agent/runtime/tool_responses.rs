...
pub(super) fn structured_user_question_from_value(value: &serde_json::Value) -> Option<String> {
    structured_user_question_from_value_at_depth(value, 0)
}
...
fn structured_user_question_from_value_at_depth(
    value: &serde_json::Value,
    depth: usize,
) -> Option<String> {
    if depth > 4 {
    ...
    for path in [
        ...
        if let Some(question) = structured_string_path(value, path) {
    ...
    object
        .get("result")
        .and_then(parse_nested_structured_value)
        .and_then(|nested| structured_user_question_from_value_at_depth(&nested, depth + 1))
}

fn structured_string_path(value: &serde_json::Value, path: &[&str]) -> Option<String> {
...
pub(super) fn structured_status_outcome_state(status: &str) -> Option<StructuredToolOutcomeState> {
    ...
    if compact.starts_with("needs") {
        return Some(StructuredToolOutcomeState::NeedsInput);
    }
...
mod tests {
    ...
    #[test]
    fn needs_prefixed_machine_statuses_are_terminal_input_states() {
        for status in ["needs_source", "needs_operation", "needs_arguments"] {
            assert_eq!(
                structured_status_outcome_state(status),
                Some(StructuredToolOutcomeState::NeedsInput)
            );
    ...
    #[test]
    fn legacy_payload_wrapper_preserves_structured_needs_status_end_to_end() {
        let raw_output = format!(
            "{}{}",
            crate::runtime::TOOL_COMPLETION_MARKER,
            serde_json::json!({
                "tool": "capability_acquire",
                "status": "needs_source",
        ...
        assert_eq!(structured_tool_result_success(&rendered), Some(false));
        assert_eq!(
            structured_tool_result_outcome(&rendered).map(|report| report.state),
            Some(StructuredToolOutcomeState::NeedsInput)
        );
    }

    /// Round-3 regression: the messaging-channel / MCP / extension-pack
    /// credential handoffs used to emit raw JSON without a `tool` key, which
    /// the legacy wrapper rewrapped as "completed" — silently classifying the
    /// secure-credential prompt as SUCCESS. Each now-structured envelope must
    /// survive the REAL wrapper path as a terminal NeedsInput.
    #[test]
    fn integration_credential_handoffs_survive_legacy_wrapper_as_needs_input() {
        for tool in [
            ...
            let raw_output = format!(
                ...
                serde_json::json!({
                    ...
                    "detail": "Credentials are still required through the secure credential form.",
                    "data": {
            ...
            let payload =
                crate::runtime::ActionRuntime::tool_payload_from_legacy_output(tool, raw_output);
            let rendered =
                crate::runtime::ActionRuntime::render_tool_payload_for_legacy(tool, payload);

            assert_eq!(
                structured_tool_result_success(&rendered),
                Some(false),
                "{tool} credential handoff must not read as success"
            );
            assert_eq!(
                structured_tool_result_outcome(&rendered).map(|report| report.state),
                Some(StructuredToolOutcomeState::NeedsInput),
                "{tool} credential handoff must classify as NeedsInput"
            );
        }
    }
