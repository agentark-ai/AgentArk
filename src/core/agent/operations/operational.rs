...
- If there is no clear transferable lesson, leave heuristic empty and fill skip_reason.",
    ...
    fn parse_reflected_heuristic_payload(
    ...
    ) -> Result<Option<crate::core::knowledge::learning::ReflectedHeuristic>, String> {
        ...
        Ok(Some(crate::core::knowledge::learning::ReflectedHeuristic {
            heuristic,
            polarity,
            confidence,
            applicability,
    ...
    async fn load_prompt_fragment_bundle_by_key(
        &self,
        key: &str,
    ) -> Option<crate::core::model::prompt_fragments::PromptFragmentBundleProfile> {
        let raw = self.storage.get(key).await.ok().flatten()?;
    ...
    pub(crate) async fn run_heuristic_reflection_pass(
        ...
        if !crate::core::knowledge::learning::load_learning_enabled(&self.storage).await {
            return Ok(HeuristicReflectionPassStats::default());
        }
        let cap =
            crate::core::knowledge::learning::load_learning_queue_cap(&self.storage).await as u64;
        let runs = self
            .storage
        ...
        for run in runs {
            ...
            match Self::parse_reflected_heuristic_payload(&payload) {
                Ok(Some(heuristic)) => {
                    let outcome =
                        crate::core::knowledge::learning::upsert_reflected_heuristic_lesson(
                            &self.storage,
                            &run,
                            &heuristic,
                        )
    ...
    pub(crate) async fn active_prompt_fragment_bundle_for_message(
        ...
        let mut selected = self
            ...
            .await
            .unwrap_or_else(crate::core::model::prompt_fragments::default_prompt_fragment_bundle);

        let canary_state_raw = self
            .storage
            .get(crate::core::model::prompt_fragments::PROMPT_FRAGMENT_BUNDLE_CANARY_STATE_KEY)
            .await
            .ok()
            .flatten();
        if let Some(raw) = canary_state_raw {
