...
impl Agent {
    ...
    async fn unlink_linked_background_tasks(&self, task_ids: &[String]) -> usize {
        let mut unlinked = 0usize;
        for task_id in task_ids {
            let Ok(id) = uuid::Uuid::parse_str(task_id) else {
                continue;
            };
            let Some(serialized_arguments) = ({
                let mut tasks = self.tasks.write().await;
                let Some(task) = tasks.get_mut(id) else {
                    continue;
                };
                task.arguments = background_session::set_background_session_id_in_automation(
                    &task.arguments,
                    None,
                );
            ...
            if self
                .storage
                .update_task(task_id, None, Some(serialized_arguments), None, None)
                .await
                .is_ok()
            {
                unlinked += 1;
    ...
    pub(super) async fn handle_background_session_manage(
        ...
        match operation.as_str() {
            ...
            "delete" => {
                task_changes = self
                    .unlink_linked_background_tasks(&session.linked_task_ids)
                    .await;
                for watcher_id in &session.linked_watcher_ids {
                    if let Ok(id) = uuid::Uuid::parse_str(watcher_id) {
                        if self
                            .watcher_manager
                            .set_background_session_id(id, None)
                            .await
                        {
                            watcher_changes += 1;
                        }
                ...
                status_detail = format!(
                    "Deleted background session{} and unlinked {} task(s) and {} watcher(s).",
