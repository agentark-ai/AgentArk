...
mod saved_memory_sensitivity_tests {
    ...
    #[test]
    fn semantic_memory_review_prompt_allows_bounded_personal_context() {
        let candidate = serde_json::json!({
            "key": "upcoming_personal_commitment",
            "value": "User has a bounded future personal commitment.",
            "kind": "ephemeral_context",
            "category": "ephemeral_context",
            "durability": "temporary",
            "scope": "conversation",
            "source_support": "explicit_source_message",
            "source_evidence": "I have a personal commitment next month",
            "confidence": 0.9,
            "reason": "The source message explicitly supplies bounded personal context."
        });

        let prompt = build_user_memory_candidate_semantic_review_prompt(
            "I have a personal commitment next month.",
            "(none)",
            &candidate,
        );

        assert!(prompt.contains("temporary/situational"));
        assert!(prompt.contains("future personal plans or commitments"));
        assert!(prompt.contains("until they expire or need review"));
    }
    ...
    fn merged_phrasing_history_is_bounded_and_normalized() {
        ...
        for index in 0..(crate::core::knowledge::memory_dedup::MAX_MERGED_PHRASINGS + 2) {
        ...
        assert_eq!(
            merged.len(),
            crate::core::knowledge::memory_dedup::MAX_MERGED_PHRASINGS
        );
        assert_eq!(
            merged
                .first()
...
pub(super) fn normalize_user_fact_key(raw: &str) -> Option<String> {
    crate::core::knowledge::memory_schema::normalize_memory_slot_key(raw, 80)
}
...
pub(super) fn append_learned_user_memory_merged_phrasing(
    ...
    while items.len() > crate::core::knowledge::memory_dedup::MAX_MERGED_PHRASINGS {
        items.remove(0);
    }
    redacted_secret
}
...
pub(super) fn normalize_learned_user_memory_category(
    ...
    crate::core::knowledge::memory_schema::normalize_memory_category(raw_category, semantic_kind)
}
...
pub(super) fn learned_user_memory_topics(
    ...
    crate::core::knowledge::memory_schema::normalize_memory_topics(item.metadata.get("topics"), 8)
}
...
fn saved_user_memory_category_weight(category: &str, scope_rank: u8) -> f32 {
    match category {
        crate::core::knowledge::memory_schema::MEMORY_CATEGORY_PROFILE_FACT => 0.34,
        crate::core::knowledge::memory_schema::MEMORY_CATEGORY_ASSISTANT_PREFERENCE => 0.32,
        crate::core::knowledge::memory_schema::MEMORY_CATEGORY_WORK_PREFERENCE
        | crate::core::knowledge::memory_schema::MEMORY_CATEGORY_PROJECT_DOMAIN
        | crate::core::knowledge::memory_schema::MEMORY_CATEGORY_KNOWLEDGE
        | crate::core::knowledge::memory_schema::MEMORY_CATEGORY_EPHEMERAL_CONTEXT
            if scope_rank > 0 =>
        {
            0.18
        }
...
fn saved_user_memory_candidate(
    ...
    if crate::core::knowledge::memory_schema::memory_category_is_ephemeral(category)
    ...
    if crate::core::knowledge::memory_schema::memory_category_requires_topical_relevance(category)
        && !saved_user_memory_is_context_scoped(item, project_id, conversation_id)
...
pub(super) async fn build_saved_user_facts_context_from_storage(
    ...
    for candidate in candidates {
        let cap =
            crate::core::knowledge::memory_schema::memory_category_prompt_cap(candidate.category);
...
pub(super) fn build_user_memory_capture_empty_retry_prompt(
    ...
    format!(
        "The previous memory extraction produced no saveable memory operations or returned an underspecified payload.\n\nRequired JSON shape:\n{response_shape}\n\nPrevious response preview:\n{previous_response_preview}\n\nRe-evaluate the source semantically. Determine whether the user supplied any durable self-information, temporary/situational self-context, preferences, workflow constraints, user-authored current-state facts worth carrying forward, or retractions. Mixed-intent turns, greetings, questions, and extra context do not cancel saveable memory content stated in the same turn. Do not recover assistant-authored status, tool output, integration setup state, or completed-work records as memory. Return JSON only, and always include both `memories` and `retractions` arrays even when one or both are empty.\n\nSource extraction task:\n{original_prompt}",
        response_shape = response_shape,
...
pub(super) fn build_user_memory_capture_empty_verdict_prompt(
    ...
    format!(
        "The previous user-memory extraction ended in an empty decision.\n\nPrevious empty extraction preview:\n{previous_response_preview}\n\nReturn JSON only with this shape:\n{{\"has_durable_memory\":false,\"confidence\":0.0,\"reason\":\"brief rationale\"}}\n\nRules:\n- Set has_durable_memory=true only when the source user message contains saveable user memory worth retaining beyond this turn.\n- Saveable user memory includes identity, stable preferences, operating rules, workflow constraints, temporary/situational self-context, meaningful user-authored current-state facts, relationships, goals, or explicit retractions.\n- Assistant-authored status, tool output, integration setup state, and completed-work records are not durable user memory.\n- Decide from meaning and context. Do not use fixed phrases, literal wording checks, regular expressions, or keyword lists.\n- Mixed-intent turns still count: a self-statement remains saveable even if the same message also asks a question, greets the assistant, or continues with another request.\n- Set has_durable_memory=false only when you are confident the source truly contains nothing worth storing.\n- confidence must be between 0.0 and 1.0.\n- reason should briefly describe the saveable meaning you found, or why the turn truly contains nothing saveable.\n\nSource extraction task:\n{original_prompt}",
        previous_response_preview = previous_response_preview,
        original_prompt = original_prompt
    )
}
...
pub(super) fn build_user_memory_capture_focused_recovery_prompt(
    ...
    format!(
        "A semantic review concluded that the previous empty extraction likely missed saveable user memory.\n\nReview reason:\n{review_reason}\n\nPrevious empty extraction preview:\n{previous_response_preview}\n\nRequired JSON shape:\n{response_shape}\n\nRecover the missed saveable memory operations from meaning. Decide semantically from the source user message, using dialogue and saved facts only as context. Do not use fixed phrases, exact wording checks, regular expressions, or keyword rules. If the source contains durable self-information, temporary/situational self-context, preferences, workflow constraints, meaningful user-authored current-state facts, or retractions, emit them. Do not recover assistant-authored status, tool output, integration setup state, or completed-work records as memory. Return JSON only, and keep both `memories` and `retractions` arrays present. Only return empty arrays if you are genuinely confident there is still nothing saveable to store.\n\nSource extraction task:\n{original_prompt}",
...
pub(super) fn format_learned_user_memory_for_prompt(
    ...
    qualifiers.push(
        crate::core::knowledge::memory_schema::memory_category_label(learned_user_memory_category(
            item,
        ))
        .to_string(),
    );
...
impl UserMemoryCaptureWorker {
            ...
            match operation.operation_type.as_str() {
                "add" | "update" => {
                    ...
                    let operation_topics =
                        crate::core::knowledge::memory_schema::normalize_memory_topics(
    ...
    pub(super) async fn upsert_learned_user_memory(
        ...
        if let Some(current) = exact_active_row {
            ...
            let embedding = match self.embedding_client.as_deref() {
                Some(embedder) if content_changed || current.embedding.is_none() => {
                    let embed_text =
                        crate::core::knowledge::memory_dedup::embeddable_text_from_content(
                            &content,
                        )
                        .to_string();
        ...
        if let Some(embedder) = self.embedding_client.as_deref() {
            let judge =
                crate::core::knowledge::memory_dedup::LlmEquivalenceJudge::new(self.llm.clone());
            let candidate = crate::core::knowledge::memory_dedup::MergeCandidate {
                kind: normalized_kind.to_string(),
                scope: scope.to_string(),
                project_id: scoped_project_id.map(str::to_string),
                conversation_id: scoped_conversation_id.map(str::to_string),
            ...
            match crate::core::knowledge::memory_dedup::attempt_absorb_into_canonical(
                &self.storage,
                &txn,
                embedder,
                &judge,
            ...
            .await
            {
                Ok(crate::core::knowledge::memory_dedup::AbsorbOutcome::Absorbed {
                    canonical_id,
                    ..
                }) => {
                    if let Err(error) = txn.commit().await {
                ...
                Ok(crate::core::knowledge::memory_dedup::AbsorbOutcome::Insert { embedding }) => {
