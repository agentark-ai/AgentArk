...
pub const DURABLE_OBSERVATION_RESULT_FIELD: &str = "durable_observation_result";
pub const DURABLE_OBSERVATION_READY_FIELD: &str = "ready_for_change_detection";
pub const DURABLE_OBSERVATION_TARGETS_FIELD: &str = "targets";
pub const DURABLE_OBSERVATION_SOURCE_FIELD: &str = "source";
pub const DURABLE_OBSERVATION_COMPARABLE_FIELD: &str = "comparable";
pub const DURABLE_OBSERVATION_VALUES_FIELD: &str = "observed_values";
pub const DURABLE_OBSERVATION_BLOCKING_GAPS_FIELD: &str = "blocking_gaps";
...
pub fn durable_observation_result_contract() -> Value {
    let mut target_properties = serde_json::Map::new();
    target_properties.insert(
        "id".to_string(),
        serde_json::json!({
            "type": "string",
            "description": "Stable identifier for this observed target."
        }),
    );
    target_properties.insert(
        DURABLE_OBSERVATION_SOURCE_FIELD.to_string(),
        serde_json::json!({
            "type": ["object", "string"],
            "description": "Stable source metadata or locator for the observed target."
        }),
    );
    target_properties.insert(
        DURABLE_OBSERVATION_COMPARABLE_FIELD.to_string(),
        serde_json::json!({
            "const": true,
            "description": "True only when observed_values can be compared with future runs."
        }),
    );
    target_properties.insert(
        DURABLE_OBSERVATION_VALUES_FIELD.to_string(),
        serde_json::json!({
            "type": ["object", "array", "string", "number", "boolean"],
            "description": "Non-empty stable observed facts to compare on later runs; not a prose summary."
        }),
    );
    target_properties.insert(
        DURABLE_OBSERVATION_BLOCKING_GAPS_FIELD.to_string(),
        serde_json::json!({
            "type": "array",
            "description": "Empty when ready. If not ready, list missing source or observation facts."
        }),
    );

    let mut root_properties = serde_json::Map::new();
    root_properties.insert(
        DURABLE_OBSERVATION_RESULT_FIELD.to_string(),
        serde_json::json!({
            "const": true
        }),
    );
    root_properties.insert(
        DURABLE_OBSERVATION_READY_FIELD.to_string(),
        serde_json::json!({
            "type": "boolean",
            "description": "True only when every target is comparable and has non-empty observed_values."
        }),
    );
    root_properties.insert(
        ...
        serde_json::json!({
            "type": "string",
            "description": "ISO 8601 timestamp for when the state was observed."
        }),
    );
    root_properties.insert(
        DURABLE_OBSERVATION_TARGETS_FIELD.to_string(),
        serde_json::json!({
            "type": "array",
            "minItems": 1,
            "items": {
                ...
                "required": [
                    "id",
                    DURABLE_OBSERVATION_SOURCE_FIELD,
                    DURABLE_OBSERVATION_COMPARABLE_FIELD,
                    DURABLE_OBSERVATION_VALUES_FIELD,
                    DURABLE_OBSERVATION_BLOCKING_GAPS_FIELD
                ],
    ...
    root_properties.insert(
        ...
        serde_json::json!({
            "type": "string",
            "description": "Brief human-readable summary. This must not be the only comparable observation."
        }),
    );
    ...
    serde_json::json!({
        "type": "object",
        "required": [
            DURABLE_OBSERVATION_RESULT_FIELD,
            DURABLE_OBSERVATION_READY_FIELD,
            DURABLE_OBSERVATION_TARGETS_FIELD
        ],
        "properties": root_properties,
        "commit_rule": "Commit only when durable_observation_result=true, ready_for_change_detection=true, every target has comparable=true, source is non-empty, observed_values is non-empty, and blocking_gaps is empty.",
        "not_ready_rule": "When comparable observations are missing, return the same field shape with ready_for_change_detection=false, comparable=false for affected targets, and blocking_gaps listing the missing facts."
    })
}
...
pub fn validate_change_detection_observation_payload(payload: &Value) -> Result<(), String> {
    let Some(object) = payload.as_object() else {
        return Err(durable_observation_contract_error(vec![
            "result must be a structured readiness JSON object".to_string(),
        ]));
    };

    let mut violations = Vec::new();
    if object
        .get(DURABLE_OBSERVATION_RESULT_FIELD)
    ...
    if object
        .get(DURABLE_OBSERVATION_READY_FIELD)
        .and_then(Value::as_bool)
        != Some(true)
    {
        violations.push(format!(
            "`{}` must be true before committing a change-detection baseline",
            DURABLE_OBSERVATION_READY_FIELD
        ));
    }

    match object
        .get(DURABLE_OBSERVATION_TARGETS_FIELD)
        .and_then(Value::as_array)
    {
        Some(targets) if !targets.is_empty() => {
            for (index, target) in targets.iter().enumerate() {
                let label = durable_observation_target_label(index, target);
                let Some(target_object) = target.as_object() else {
                    violations.push(format!("{label} must be an object"));
                    continue;
                };
                if target_object
                    .get(DURABLE_OBSERVATION_COMPARABLE_FIELD)
                    .and_then(Value::as_bool)
                    != Some(true)
                {
                    violations.push(format!(
                ...
                if !durable_observation_value_is_material(
                    target_object
                        .get(DURABLE_OBSERVATION_SOURCE_FIELD)
                        .unwrap_or(&Value::Null),
                ) {
                    violations.push(format!(
                        "{label}.{} must identify the observed source",
                        DURABLE_OBSERVATION_SOURCE_FIELD
                    ));
                }
                if !durable_observation_value_is_material(
                    target_object
                        .get(DURABLE_OBSERVATION_VALUES_FIELD)
                        .unwrap_or(&Value::Null),
                ) {
                    violations.push(format!(
                        "{label}.{} must contain comparable observed values",
                        DURABLE_OBSERVATION_VALUES_FIELD
                    ));
                }
                if target_object
                    .get(DURABLE_OBSERVATION_BLOCKING_GAPS_FIELD)
                    .and_then(Value::as_array)
                    .is_some_and(|gaps| !gaps.is_empty())
                {
                    violations.push(format!(
                        "{label}.{} must be empty before the result is committed",
                        DURABLE_OBSERVATION_BLOCKING_GAPS_FIELD
                    ));
                }
            }
    ...
    if violations.is_empty() {
        ...
        Err(durable_observation_contract_error(violations))
    }
}

pub fn arguments_require_change_detection_observation(arguments: &Value) -> bool {
    let Some(result_contract) = arguments
    ...
    result_contract
        .get("properties")
        .and_then(|value| value.get(DURABLE_OBSERVATION_RESULT_FIELD))
        .is_some()
        || result_contract
            .get("required")
            .and_then(Value::as_array)
            .is_some_and(|required| {
                required
                    .iter()
                    .any(|field| field.as_str() == Some(DURABLE_OBSERVATION_RESULT_FIELD))
            })
}

fn durable_observation_contract_error(violations: Vec<String>) -> String {
    format!(
        "Durable semantic observation is not ready to commit as a change-detection baseline. Return structured readiness JSON satisfying this contract: {}. Violations: {}",
        durable_observation_result_contract(),
        violations.join("; ")
    )
}

fn durable_observation_target_label(index: usize, target: &Value) -> String {
    target
        .get("id")
        .and_then(Value::as_str)
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .map(|id| format!("target[{index}] `{}`", truncate_text(id, 80)))
        .unwrap_or_else(|| format!("target[{index}]"))
}

fn durable_observation_value_is_material(value: &Value) -> bool {
    match value {
        Value::Null => false,
        Value::Bool(_) | Value::Number(_) => true,
        Value::String(text) => !text.trim().is_empty(),
        Value::Array(items) => items.iter().any(durable_observation_value_is_material),
        Value::Object(object) => object.values().any(durable_observation_value_is_material),
    }
}
...
mod tests {
    ...
    fn comparable_observation_payload() -> Value {
        let mut target = serde_json::Map::new();
        target.insert("id".to_string(), Value::String("primary".to_string()));
        target.insert(
            DURABLE_OBSERVATION_SOURCE_FIELD.to_string(),
            serde_json::json!({
                "kind": "http",
    ...
    #[test]
    fn change_detection_observation_requires_structured_readiness() {
        let error = validate_change_detection_observation_payload(&serde_json::json!({
            "summary": "reachable but not comparable"
        }))
        .expect_err("unready observation must be rejected");

        assert!(error.contains("structured readiness"));
        assert!(error.contains(DURABLE_OBSERVATION_RESULT_FIELD));
        assert!(error.contains(DURABLE_OBSERVATION_TARGETS_FIELD));
    }

    #[test]
    fn change_detection_observation_accepts_comparable_targets() {
        let payload = comparable_observation_payload();

        assert!(validate_change_detection_observation_payload(&payload).is_ok());
    }

    #[test]
    fn change_detection_observation_rejects_contract_template_as_payload() {
        let template = durable_observation_result_contract();

        assert!(validate_change_detection_observation_payload(&template).is_err());
    }

    #[test]
    fn change_detection_observation_contract_is_detected_from_arguments() {
        let arguments = serde_json::json!({
            "execution_contract": {
                "result_contract": durable_observation_result_contract()
            }
        });

        assert!(arguments_require_change_detection_observation(&arguments));
    }

    #[test]
    fn change_detection_observation_rejects_incomparable_targets() {
        let mut payload = comparable_observation_payload();
        payload[DURABLE_OBSERVATION_TARGETS_FIELD][0][DURABLE_OBSERVATION_COMPARABLE_FIELD] =
            Value::Bool(false);
        payload[DURABLE_OBSERVATION_TARGETS_FIELD][0][DURABLE_OBSERVATION_VALUES_FIELD] =
            serde_json::json!({});
        payload[DURABLE_OBSERVATION_TARGETS_FIELD][0][DURABLE_OBSERVATION_BLOCKING_GAPS_FIELD] =
            serde_json::json!(["values unavailable"]);
        let error = validate_change_detection_observation_payload(&payload)
            .expect_err("incomparable target must be rejected");

        assert!(error.contains(DURABLE_OBSERVATION_COMPARABLE_FIELD));
        assert!(error.contains(DURABLE_OBSERVATION_VALUES_FIELD));
        assert!(error.contains(DURABLE_OBSERVATION_BLOCKING_GAPS_FIELD));
    }
