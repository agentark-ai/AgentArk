...
pub async fn upsert_marketplace(
    ...
    crate::core::runtime::net::validate_public_https_url(&url)
        .await
...
async fn fetch_marketplace_text(raw_url: &str) -> Result<String> {
    let mut current = crate::core::runtime::net::validate_public_https_url(raw_url).await?;
    ...
    for _ in 0..=MARKETPLACE_MAX_REDIRECTS {
        ...
        if response.status().is_redirection() {
            ...
            current = crate::core::runtime::net::validate_public_https_url(next.as_str()).await?;
            continue;
        }
