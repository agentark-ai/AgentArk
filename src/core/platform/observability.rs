use crate::core::agent::ExecutionTrace;
...
pub fn has_observability_auth_token(
    ...
    let manager =
        crate::core::runtime::config::SecureConfigManager::new_with_data_dir(config_dir, data_dir)?;
    Ok(manager
...
fn should_export_step(step: &crate::core::agent::ExecutionStep) -> bool {
    !matches!(
        step.title.trim(),
...
fn export_step_name(step: &crate::core::agent::ExecutionStep) -> String {
...
fn build_step_attributes(
    trace: &ExecutionTrace,
    step: &crate::core::agent::ExecutionStep,
    mode: ObservabilityPrivacyMode,
) -> Vec<serde_json::Value> {
    let mut attributes = vec![
        otel_attribute_string("agentark.trace_id", trace.id.clone()),
...
fn build_otlp_trace_payload(config: &AgentConfig, trace: &ExecutionTrace) -> serde_json::Value {
    ...
    let mode = if config.deployment_mode
...
pub async fn export_test_trace(
    ...
    let trace = ExecutionTrace {
        ...
        steps: vec![crate::core::agent::ExecutionStep {
            icon: "[test]".to_string(),
