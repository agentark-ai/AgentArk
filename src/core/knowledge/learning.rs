...
fn reconciled_memory_category(item: &experience_item::Model) -> Option<&'static str> {
    ...
    let existing = item
        ...
        .map(str::trim)
        .filter(|value| !value.is_empty());
    let fallback = crate::core::knowledge::memory_schema::normalize_memory_category(
        None,
        semantic_kind.as_deref(),
    );
    let normalized = crate::core::knowledge::memory_schema::normalize_memory_category(
        existing,
        semantic_kind.as_deref(),
    );
    if existing.is_none()
        && fallback != crate::core::knowledge::memory_schema::MEMORY_CATEGORY_OTHER
    {
        return Some(fallback);
    }
    if existing == Some(crate::core::knowledge::memory_schema::MEMORY_CATEGORY_OTHER)
        && fallback != crate::core::knowledge::memory_schema::MEMORY_CATEGORY_OTHER
    {
        return Some(fallback);
    }
