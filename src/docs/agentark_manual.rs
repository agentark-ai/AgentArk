...
pub(crate) const AGENTARK_MANUAL_DOCS: &[AgentArkManualDoc] = &[
        ...
        sections: &[
            ...
            AgentArkManualSection {
                ...
                items: &[
                    ...
                    "The bridge writes a redacted run export to `/app/data/.agentark/self_evolve/gepa/runs/<run_id>/export.json`.",
                    ...
                    "Job status files move through `/app/data/.agentark/self_evolve/gepa/pending`, `running`, `completed`, and `failed`; detailed run artifacts live under `/app/data/.agentark/self_evolve/gepa/runs`.",
                ],
            },
            AgentArkManualSection {
    ...
    AgentArkManualDoc {
        title: "ArkDistill context savings",
        slug: "arkdistill-context-savings",
        tags: &[
            "arkdistill",
            "context_savings",
            "tool_output",
        ...
        summary: "ArkDistill deterministically compacts noisy tool output before the next model turn and reports aggregate saved context, saved tokens, saved cost, and savings percentage in Analytics.",
        sections: &[
            AgentArkManualSection {
                label: "what it does",
                items: &[
                    "ArkDistill runs after a tool returns structured data and before that data enters the model-visible context.",
                    "It keeps required fields such as status, ids, URLs, messages, and errors while shrinking noisy strings, repeated lines, HTML, traces, logs, browser page text, and large dumps.",
                    "It is deterministic profile logic, not a live model summarizer.",
                    "On noisy outputs, expect context savings often around 60-90%; compact JSON or already-small integration responses usually save less.",
                ],
            },
            AgentArkManualSection {
                label: "where code lives",
                items: &[
                    "`src/core/agent/ark_distill.rs` owns profile schema, deterministic distillation, validation, replay fixtures, and ArkDistill GEPA candidate evaluation.",
                    "`src/core/agent/spine.rs` normalizes tool results, applies ArkDistill before model context, and writes `arkdistill_tool_output` operational logs.",
                    "`src/channels/http/arkdistill_analytics.rs` aggregates operational logs into `/analytics/llm` response data.",
                    "`frontend/src/components/pages/AnalyticsPage.tsx` shows aggregate ArkDistill saved percentage, saved tokens, and estimated prompt cost avoided.",
                    "`assets/docs/arkdistill.md` is the user-facing technical reference.",
                ],
            },
            AgentArkManualSection {
                label: "state and storage",
                items: &[
                    "No database migration is required for ArkDistill. It uses existing KV storage and existing `operational_logs.payload`, so schema version remains 1.",
                    "Active profile KV key: `tool_output_distill_profile_v1`.",
                    "Rollback snapshot KV key: `tool_output_distill_profile_baseline_snapshot_v1`.",
                    "Last import/evaluation result KV key: `tool_output_distill_profile_last_result_v1`.",
                    "Per-tool stats are stored as `operational_logs` rows with `event_type = 'arkdistill_tool_output'`.",
                ],
            },
            AgentArkManualSection {
                label: "analytics fields",
                items: &[
                    "`/analytics/llm` includes `arkdistill.totals`, `arkdistill.series`, and `arkdistill.by_tool`.",
                    "`arkdistill.totals.saved_chars` is the aggregate character reduction for the selected time window.",
                    "`arkdistill.totals.estimated_saved_tokens` is the aggregate estimated token reduction.",
                    "`arkdistill.totals.estimated_prompt_cost_saved_usd` is estimated prompt cost avoided when pricing metadata is available.",
                    "`arkdistill.totals.savings_percent` is computed as total saved characters divided by total original characters for the selected time range.",
                    "`arkdistill.by_tool` ranks tool/action surfaces by saved estimated tokens.",
                ],
            },
            AgentArkManualSection {
                label: "debugging queries",
                items: &[
                    "Prefer the Analytics UI or `/analytics/llm` when answering normal user questions about whether ArkDistill is saving context.",
                    "For code/debug work, use `Storage::list_operational_logs_by_event(crate::core::ARKDISTILL_EVENT_TYPE, limit)` because it returns decrypted payloads through the storage layer.",
                    "Direct SQL shape: `SELECT created_at, tool_name, outcome, payload FROM operational_logs WHERE event_type = 'arkdistill_tool_output' ORDER BY created_at DESC LIMIT 50;`.",
                    "Raw database reads may show encrypted payload fields depending on storage configuration; the application storage helper is the safer diagnostic path.",
                    "If there are no `arkdistill_tool_output` rows, either no tool output has been compacted in the selected window or the tool results were already small enough to skip logging.",
                    "If savings percent is unexpectedly low, inspect `by_tool` first. Compact structured JSON responses will naturally produce lower savings than noisy browser pages or logs.",
                ],
            },
            AgentArkManualSection {
                label: "Evolve and GEPA",
                items: &[
                    "GEPA exports the current ArkDistill profile as the `arkdistill_profile` surface.",
                    "Imported candidates are sanitized and replayed against synthetic noisy tool fixtures before promotion.",
                    "Candidates must preserve valid JSON and required fields while improving estimated saved tokens.",
                    "A promoted profile is written to `tool_output_distill_profile_v1`; the previous profile is saved as the baseline snapshot first.",
                    "ArkDistill profile optimization should tune structured profile rules and limits, not hardcode user phrasing.",
                ],
            },
            AgentArkManualSection {
                label: "answer rules",
                items: &[
                    "When the user asks how much context ArkDistill saved, answer from live Analytics data when available, not from the 60-90% expected noisy-output range.",
                    "When the user asks to debug ArkDistill performance, check the selected time window, `arkdistill.totals.savings_percent`, `estimated_saved_tokens`, `estimated_prompt_cost_saved_usd`, and `by_tool` contributors.",
                    "Explain low savings as tool/output dependent before treating it as a bug.",
                    "Do not claim ArkDistill changes full raw trace retention; it changes the model-visible tool output only.",
                    "Do not describe ArkDistill as prompt matching or phrase matching. It uses structured tool/action metadata and field rules.",
                ],
            },
        ],
    },
