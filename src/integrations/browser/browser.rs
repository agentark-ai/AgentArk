...
fn browser_target_is_internal_app_url(parsed: &url::Url) -> bool {
    ...
    let Ok(internal_base) = url::Url::parse(&crate::core::runtime::net::internal_api_base_url())
    else {
        return false;
    };
...
impl BrowserIntegration {
    ...
    pub async fn type_text(
        ...
        if let Some(index) = element_index {
            body["element_index"] = serde_json::json!(index);
        }
        let response = self
            .client
...
mod tests {
    ...
    fn browser_accepts_internal_app_targets_only() {
        let base = crate::core::runtime::net::internal_api_base_url();
        ...
        match action {
            ...
            "type_text" => {
                ...
                let clear = params
                    ...
                    .unwrap_or(false);
                self.type_text(sid, text, selector, element_index, clear)
                    .await?;
